...
import OPDpages.Cancel_New_AppointMent_against_doctor;
...
Cancel_New_AppointMent_against_doctor BookNewAppointment = new Cancel_New_AppointMent_against_doctor(driver);
...
public void TC_03_Verify_that_appointment_is_cancelled_succsessfull(String RemarksPar) throws InterruptedException, AWTException, IOException { 
	cancelAppointmentPage canc = new cancelAppointmentPage(driver);
	...
	public void beforeTest() throws InterruptedException, ATUTestRecorderException {
		...
		Date date = new Date();
		recorder = new ATUTestRecorder(".//Videos//","Cancel_new_appointment_against_Doctor-"+dateFormat.format(date),false);
		recorder.start();
