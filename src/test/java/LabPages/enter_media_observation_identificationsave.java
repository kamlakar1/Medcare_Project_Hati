package LabPages;


import java.awt.AWTException;
import java.awt.datatransfer.StringSelection;
import java.io.IOException;
import java.sql.Date;
import java.text.SimpleDateFormat;
import java.util.ArrayList;

import org.openqa.selenium.By;
import org.openqa.selenium.NoSuchElementException;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.interactions.Actions;
import org.openqa.selenium.support.ui.ExpectedConditions;
import org.openqa.selenium.support.ui.FluentWait;
import org.openqa.selenium.support.ui.Wait;
import org.openqa.selenium.support.ui.WebDriverWait;
import org.testng.Assert;
import java.time.Duration;




public class  enter_media_observation_identificationsave
{
	//private static final By UploadImhe = null;
	WebDriver driver=null;

	//Race
	By RaceClick=By.cssSelector("mat-select[ng-reflect-name='race']");
	By RaceSelOpt=By.xpath("//span[contains(text(),'Refugee')]");

	//By Documenttab4Click=By.cssSelector("div[aria-controls='mat-tab-content-1-3']");

	By Documenttab4Click=By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[3]/form/mat-tab-group/mat-tab-header/div/div/div/div[4]");



	//By Search=By.xpath("mat-icon[mattooltip='Search']");
	By mainmenu=By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/app-header/nav/mat-icon[1]");
	By mainmenu2=By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/app-header/nav/mat-icon[2]");
	By MainMenuTooltip=By.xpath("/html/body/div[2]");
	By OPDMenu=By.xpath("//*[@id=\"mat-menu-panel-1\"]/div/div/div[1]/div[1]/div[1]/div/img");
	//	By SideBarMenu=By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/ng-sidebar/aside");
	//By RegistrationMenu=By.xpath("//span[contains(text(),'Registration')]");
	By RegistrationMenu=By.xpath("//*[@id='2004']");

	By subRegistration=By.xpath("//*[@id='2005']");
	By PlusButton=By.xpath("//*[@id='Add Patient']");	

	//Image Uploading
	//Clicking for Uploading
	By ClickImageUplaod=By.xpath("//*[@id=\"registration\"]/form/div[1]/div/div[1]/div/img");
	By UploadImageClick=By.xpath("//*[@id=\"btnUploadPhoto\"][3]");
	By SaveButtonClickIamge=By.xpath("//*[@id=\"mat-dialog-0\"]/app-camera-dialog/div/div[2]/div[2]/div[2]/button[3]");

	//Basic Information

	//for all search same xpath
	By AllDropDownSearch=By.cssSelector("input[aria-label='dropdown search']");

	//Locator of Primary Identification Drop down
	By PrimaryIdentificationClick=By.cssSelector("mat-select[formcontrolname='primaryId']");
	By PrimaryIdentificationselOpt=By.xpath("//span[contains(text(),'Passport')]");

	//Primary Id Number
	By PrimaryIdNumber=By.cssSelector("input[formcontrolname='primaryNumber']");
	//ExpirayDate
	By ExpirayDate=By.cssSelector("input[formcontrolname='expiryDate']");

	//Issuing country
	By Issuingcountryclick=By.cssSelector("mat-select[ng-reflect-name='issuedCountryPrimary']");
	By IssuingcountrySelOpt=By.xpath("//span[contains(text(),'Malaysia')]");

	//Secondary Identification 
	By SecondaryIdentificationClick=By.cssSelector("mat-select[ng-reflect-name='secondaryId']");
	By SecondaryIdentificationSelOpt=By.xpath("//span[contains(text(),'Work Permit')]");

	//Secondary ID Number
	By SecondaryIDNumClick=By.cssSelector("input[ng-reflect-placeholder='Secondary Id Number']");
	//Secondary Expiray Date
	By SecExpirayDate=By.cssSelector("input[formcontrolname='secondaryExpirydate']");

	//Faculty selection
	By FacultyClick=By.cssSelector("mat-select[ng-reflect-name='faculty']");
	By FacultySelOpt=By.xpath("//span[contains(text(),'FAKULTI KEMANUSIAAN, SENI DAN WARISAN')]");	

	//Personal Information

	//Title
	//By TitleClick=By.cssSelector("mat-select[ng-reflect-name='prefix']");
	//By TitleSelOpt=By.xpath("//span[contains(text(),'Tan Sri Dato Seri')]");
	//Full Name
	By Fullname=By.cssSelector("input[data-placeholder='Full Name']");
	//gender
	By GenderClick=By.cssSelector("mat-select[ng-reflect-name='gender']");
	By GenderSelOpt=By.xpath("//span[contains(text(),'FEMALE')]");

	//martial status
	By MaritalStatusClick=By.cssSelector("mat-select[ng-reflect-name='martial']");
	By MaritalStatusSelOpt=By.xpath("//span[contains(text(),' Married ')]");

	//Religion
	By ReliginClick=By.cssSelector("mat-select[ng-reflect-name='religion']");
	By ReliginClickSelOpt=By.xpath("//span[contains(text(),'Islam')]");

	//Date of Birth
	By DateOfBirth=By.cssSelector("input[ng-reflect-placeholder='Date of Birth']");

	//Deceased
	By Deceased=By.xpath("//*[@id='mat-checkbox-8']/label");

	//Patient category
	//By PatientCategoryClick=By.cssSelector("mat-select[ng-reflect-name='patientCategory']");
	//By PatientCategorySelOpt=By.xpath("//span[contains(text(),'Senior Citizen')]");

	//Preferred Language
	By PreferredLanguageClick=By.cssSelector("mat-select[ng-reflect-name='preferredLanguage']");
	By PreferredLanguageSelOpt=By.xpath("//span[contains(text(),'Mandarin')]");

	//nationality
	By NationalityClick=By.cssSelector("mat-select[ng-reflect-name='nation']");
	By NationalitySelOpt=By.xpath("//span[contains(text(),'Albania')]");

	//Mother name
	By MotherNameCLick=By.cssSelector("input[formcontrolname='motherName']");

	//Education
	By EducationClick=By.cssSelector("mat-select[ng-reflect-name='education']");
	By EducationSelOpt=By.xpath("//span[contains(text(),'Bachelor of Medicine and Bachelor of Surgery (MBBS)')]");

	//Occupation
	By OccupationClick=By.cssSelector("mat-select[ng-reflect-name='occupation']");
	By OccupationSelOpt=By.xpath("//span[contains(text(),'Mining')]");

	//Race
	//By RaceClick=By.cssSelector("mat-select[ng-reflect-name='race']");
	//By RaceSelOpt=By.xpath("//span[contains(text(),'Chinese')]");

	//Ethnic
	By EthnicClick=By.cssSelector("mat-select[ng-reflect-name='ethnic']");
	By EthnicSelOpt=By.xpath("//span[contains(text(),' Malay ')]");

	//Birth Place
	By BirthPlaceClick=By.cssSelector("input[formcontrolname='birthplace']");

	//Blood Group
	By BloodGroupClick=By.cssSelector("mat-select[ng-reflect-name='bloodGroup']");
	By BloodGroupSelOpt=By.xpath("//span[contains(text(),'A RhD positive (A+)')]");

	//Source
	By SourceClick=By.cssSelector("mat-select[ng-reflect-name='sourceType']");
	By SourceSelOpt=By.xpath("//span[contains(text(),'Exhibition')]");

	//Employment Status
	By EmploymentStatusClick=By.cssSelector("mat-select[ng-reflect-name='employmentStatus']");
	By EmploymentStatusSelOpt=By.xpath("//span[contains(text(),'Employed')]");

	//Patient Notes
	By PatientNotesClick=By.cssSelector("input[formcontrolname='patientNotes']");

	//Tab 1 Address and contact Details
	By AddressAndContactDetail=By.cssSelector("div[aria-controls='mat-tab-content-2-0']");

	//Address
	//Residential Address
	By ResAddress=By.cssSelector("input[ng-reflect-name='addressPrimary']");

	//Country
	By countryClick=By.cssSelector("mat-select[ng-reflect-name='countryPrimary']");
	By countrySelOpt=By.xpath("//span[contains(text(),' Malaysia ')]");	 

	//State
	By StateClick=By.cssSelector("mat-select[ng-reflect-name='statePrimary']");
	By StateSelOpt=By.xpath("//span[contains(text(),'Negeri Sembilan')]");

	//City
	By cityClick=By.cssSelector("mat-select[ng-reflect-name='cityPrimary']");
	By citySelopt=By.xpath("//span[contains(text(),'Seremban')]");

	//Area
	By AreaClick=By.cssSelector("mat-select[ng-reflect-name='subDistrictPrimary']");
	By AreaSelOpt=By.xpath("//span[contains(text(),'None')]");

	//PostCode
	By PostCode=By.cssSelector("input[formcontrolname='pincodePrimary']");

	//Contact Information	

	//Country Code
	By CountryCodeClick=By.cssSelector("mat-select[ng-reflect-name='countryCodemobile']");
	By CountryCodeSelOpt=By.xpath("//span[contains(text(),' +213 ')]");
	//Mobile Number
	By MobileNumberClick=By.cssSelector("input[formcontrolname='mobileNumber']");

	//Relation
	//	By RelationClick=By.cssSelector("mat-select[ng-reflect-name='contactRelation']");
	//	By RelationSelOpt=By.xpath("//span[contains(text(),'Wife')]");
	//Email
	By EmailClick=By.cssSelector("input[formcontrolname='emailId']");
	//Personal Fax
	By PersonalFax=By.cssSelector("input[formcontrolname='personalFax']");
	//Work Fax
	By WorkFax=By.cssSelector("input[formcontrolname='workFax']");
	//Work Contact Number
	By WorkContactNo=By.cssSelector("input[formcontrolname='workContact']");
	//Home contact Number
	By HomeContactNo=By.cssSelector("input[formcontrolname='homeContact']");

	//Tab 2 Employment details Tab

	By EmploymentDetailsTab=By.cssSelector("div[aria-controls='mat-tab-content-2-1']");
	//Employeer
	By EmployeerClick=By.cssSelector("mat-select[ng-reflect-name='employerName']");
	By EmployeerSelOpt=By.xpath("//span[contains(text(),'Ayesha Sudhir Lande')]");

	//Employment No
	By EmploymentNo=By.cssSelector("input[formcontrolname='employmentNo']");

	//Working from
	By WorkingFrom=By.cssSelector("input[formcontrolname='workingFrom']");

	//Contact No
	By ContactNo=By.cssSelector("input[formcontrolname='employmentContactNo']");

	//Employment Address
	By EmpAdd=By.cssSelector("input[formcontrolname='employmentAddress']");

	//Country
	By Country2Click=By.cssSelector("mat-select[ng-reflect-name='employmentCountry']");
	By Country2SelOpt=By.xpath("//span[contains(text(),' Malaysia ')]");

	//State
	By State2Click=By.cssSelector("mat-select[ng-reflect-name='employmentState']");
	By State2SelOpt=By.xpath("//span[contains(text(),'Negeri Sembilan')]");

	//City
	By City2Click=By.cssSelector("mat-select[ng-reflect-name='employmentCity']");
	By City2SelOpt=By.xpath("//span[contains(text(),'None')]");

	//PostCode
	By PostCode2=By.cssSelector("input[formcontrolname='employmentPostCode']");

	//Tab3 Document 
	//Documents
	By Documenttab3Click=By.cssSelector("div[aria-controls='mat-tab-content-2-2']");
	//Save Button
	//By SaveButton=By.cssSelector("mat-icon[ng-reflect-message='Save']");
	//OK Dialog
	By OkDialog=By.cssSelector("mat-icon[ng-reflect-message='Yes']");
	//Upload Button
	By UploadButton=By.cssSelector("mat-icon[ng-reflect-message='Upload Document']");
	//Choose Files
	By ChooseFiles=By.cssSelector("input[id='fileUpload']");
	//File upload last
	By FileUploadFinalclick=By.cssSelector("mat-icon[ng-reflect-message='Upload']");
	//Back Button
	By Back=By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[4]/button[1]/span[1]/mat-icon");


	By CurrentOrder = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-phlebotomy/mat-tab-group/mat-tab-header/div/div/div/div[1]");
	By currentmrn = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-phlebotomy/mat-tab-group/div/mat-tab-body[1]/div/div/mat-table/mat-row[1]/mat-cell[4]");
	By checkbox = By.xpath(
			"/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-phlebotomy/mat-tab-group/div/mat-tab-body[1]/div/div/mat-table/mat-accordion/mat-expansion-panel/mat-expansion-panel-header/span[1]/mat-panel-title/mat-checkbox");
	// By print =
	// By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-phlebotomy/div[2]/mat-grid-list/div/mat-grid-tile[4]/div/section/div/div/div[1]/button/span[1]/mat-icon");

	By closee = By.xpath("/html/body/div[2]/div[2]/div/mat-dialog-container/app-barcode-scanner/div/form/div[1]/span/mat-icon");

	By sampletrack = By.id("22662");

	By cicrcle = By.xpath(
			"/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-phlebotomy/div[2]/mat-grid-list/div/mat-grid-tile[3]/div/mat-radio-group/mat-radio-button[2]/label/span[1]/span[1]");

	By printandsubmit = By.cssSelector("button[ng-reflect-message='Print and Sample']");

	By saveandsub = By.cssSelector("button[value='submit']");

	By Cancel = By.cssSelector("button[ng-reflect-message='Cancel']");

	By Expand = By.cssSelector("mat-icon[ng-reflect-message='Expand']");
	By clickonedit = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-incubation/div[3]/mat-table/mat-row/mat-cell[14]/mat-icon");


	By mrnnnm = By.cssSelector("input[ng-reflect-placeholder='Patient MRN']");
	By sercahghh = By.cssSelector("button[ng-reflect-message='Search']");
	By getmrn = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-incubation/div[3]/mat-table/mat-row[1]/mat-cell[1]");
	By Microbilogy = By.id("1567");
	By MediaObservation = By.id("15677");

	// =======================================
	// Encounter details

	By EncounterType = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[1]/div/div/form/div/div[1]/div[2]/div[1]/app-master-form-field/div/mat-form-field/div/div[1]/div[3]/mat-select");
	By Enttypeopt = By.xpath("//span[contains(text(),'Follow up')]");
	By Speciality = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[1]/div/div/form/div/div[1]/div[2]/div[2]/mat-form-field/div/div[1]/div[3]/mat-select/div/div[1]/span");
	By septopt = By.xpath("//span[contains(text(),' General Surgery')]");
	By Doctor = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[1]/div/div/form/div/div[1]/div[3]/div[1]/mat-form-field/div/div[1]/div[3]/mat-select/div/div[1]/span");
	By doctopt = By.xpath("//span[contains(text(),' Dr Edward Kerry')]");
	By Clicnic = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[1]/div/div/form/div/div[1]/div[3]/div[2]/app-master-form-field/div/mat-form-field/div/div[1]/div[3]/mat-select/div/div[1]");
	By clcincopt = By.xpath("//span[contains(text(),' Consultation Room 1')]");
	By PatintMRN = By.cssSelector("input[ng-reflect-placeholder='Patient MRN']");
	By save = By.cssSelector("mat-icon[ng-reflect-message='Save']");
	By Yes = By.xpath("mat-icon[ng-reflect-message='Yes']");

	// ================
	// Encounter details

	By EncounterList = By.xpath("//span[contains(text(),'Encounter List')]");

	By EncounterTypeClick = By.cssSelector("mat-select[ng-reflect-name='encounterType']");
	By Search = By.cssSelector("input[aria-label='dropdown search']");
	By EncounterTypeSelOpt = By.xpath("//span[contains(text(),'New Case')]");
	// Specialty
	By SpecialityClick = By.cssSelector("mat-select[ng-reflect-name='department']");
	// By
	// SpecialitySearch=By.xpath("//*[@id=\"mat-option-4194\"]/span/ngx-mat-select-search/div/input");
	By SpecilitySelOPt = By.xpath("//span[contains(text(),'General Surgery')]");
	// Doctor
	By DoctorClick = By.cssSelector("mat-select[ng-reflect-name='doctor']");
	// By
	// DoctorSearch=By.xpath("//*[@id=\"mat-option-4190\"]/span/ngx-mat-select-search/div/input");
	By DoctorSelOpt = By.xpath("//span[contains(text(),'Dr Edward Kerry')]");
	// Clinic
	By ClinicClick = By.cssSelector("mat-select[ng-reflect-name='clinic']");
	// By
	// ClinicSearch=By.xpath("//*[@id=\"mat-option-4196\"]/span/ngx-mat-select-search/div/input");
	By ClinicSelOpt = By.xpath("//span[contains(text(),'Consultation Room 1')]");
	// Counter
	By CounterClick = By.cssSelector("mat-select[ng-reflect-name='cashCounter']");
	// By
	// CounterSearch=By.xpath("//*[@id=\"mat-option-4198\"]/span/ngx-mat-select-search/div/input");
	By CounterSelOpt = By.xpath("//span[contains(text(),'REGISTRATION COUNTER 1')]");
	// Encounter Remarks
	By EncounterRemarks = By.cssSelector("textarea[ng-reflect-name='remarks']");
	// Encounter Notes
	By EncounterNotes = By.cssSelector("mat-icon[ng-reflect-message='Internal Note']");
	// Add Notes
	By AddNotes = By.cssSelector("input[ng-reflect-name='notes']");
	// Add Remark Button
	By AddRemarkButton = By.cssSelector("mat-icon[ng-reflect-message='Add Remark']");
	// Close Button
	By CloseButton = By.cssSelector("mat-icon[ng-reflect-message='Close']");

	// Referral Details
	// VisitReason
	By VisitReasonClick = By.xpath(
			"/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[1]/div/div/form/div/div[2]/div[2]/div[1]/app-master-form-field/div/mat-form-field/div/div[1]/div[3]/mat-select");
	// By
	// VisitReasonSearch=By.xpath("//*[@id=\"mat-option-4200\"]/span/ngx-mat-select-search/div/input");
	By VisitReasonSelOpt = By.xpath("//span[contains(text(),'Consultation & Treatment')]");
	// Patient Category
	By PatientCategoryClick = By.cssSelector("mat-select[ng-reflect-name='patientCategory']");
	// By
	// PatientCategorySearch=By.xpath("//*[@id=\"mat-option-4202\"]/span/ngx-mat-select-search/div/input");
	By PatientCategorySelOpt = By.xpath("//span[contains(text(),'Senior Citizen')]");
	// Referral Type
	By ReferralTypeClick = By.cssSelector("mat-select[ng-reflect-name='referralType']");
	// By
	// ReferralTypeSearch=By.xpath("//*[@id=\"mat-option-4218\"]/span/ngx-mat-select-search/div/input");
	By ReferralTypeSelOpt = By.xpath("//span[contains(text(),'Emergency Referral')]");
	// Referral
	By ReferralClick = By.cssSelector("mat-select[ng-reflect-name='referral']");
	// By
	// ReferralSearch=By.xpath("//*[@id=\"mat-option-4220\"]/span/ngx-mat-select-search/div/input");
	By ReferralSelOpt = By.xpath("//span[contains(text(),\"Dato' Dr. Jefri Zain \")]");
	// Patient Notes
	// By PatientNotes12=By.xpath("//*[@id=\"mat-input-102\"]");
	// Referral Remarks
	By ReferralRemarks12 = By.cssSelector("textarea[ng-reflect-name='referralRemarks']");
	// selecting Tariff
	By TariffClick = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[1]/div/div/div[2]/table/tbody/tr/td[4]/mat-select");
	// By
	// TariffSearch=By.xpath("//*[@id=\"mat-option-4226\"]/span/ngx-mat-select-search/div/input");
	By TariffSelOpt = By.xpath("//span[contains(text(),'Self Tariff')]");

	// Tab 2 Next Of Kin
	By NextKinTabClick = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/mat-tab-header/div/div/div/div[2]/div");

	// Next Of kin Details
	// Titles
	By TitleClick = By.cssSelector("mat-select[ng-reflect-name='prefix']");
	// By
	// TitleSearch=By.xpath("//*[@id=\"mat-option-4204\"]/span/ngx-mat-select-search/div/input");
	By TitleSelOpt = By.xpath("//span[contains(text(),'Tan Sri Dato Seri')]");
	By TitleSelOpt2 = By.xpath("//span[contains(text(),'Tan Sri Datuk')]");


	// Name
	By NameClick = By.cssSelector("input[ng-reflect-name='name']");
	// Identification Type
	By IdentificationTypeClick = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[2]/div/div/form/div/div[1]/div[1]/div[2]/div[3]/app-master-form-field/div/mat-form-field/div/div[1]/div[3]/mat-select");
	// By
	// IdentificationTypeSearch=By.xpath("//*[@id=\"mat-option-4206\"]/span/ngx-mat-select-search/div/input");
	By IdentificationTypeSelOpt = By.xpath("//span[contains(text(),'Passport')]");
	// Identification Number
	By IdNumber = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[2]/div/div/form/div/div[1]/div[1]/div[3]/div[1]/mat-form-field/div/div[1]/div[3]/input");
	// Relation
	By RelationClick = By.cssSelector("mat-select[ng-reflect-name='relation']");
	// By
	// RelationSearch=By.xpath("//*[@id=\"mat-option-4208\"]/span/ngx-mat-select-search/div/input");
	By RelationSelOpt = By.xpath("//span[contains(text(),'Mother')]");
	// Phone Number
	By PhoneNumber = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[2]/div/div/form/div/div[1]/div[1]/div[3]/div[3]/mat-form-field/div/div[1]/div[3]/input");

	// Address Details
	// Address
	By AddressNClick = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[2]/div/div/form/div/div[1]/div[2]/div[2]/div[1]/mat-form-field/div/div[1]/div[3]/input");
	// Selecting Country
	By CountryNClick = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[2]/div/div/form/div/div[1]/div[2]/div[2]/div[2]/app-master-form-field/div/mat-form-field/div/div[1]/div[3]/mat-select");
	// By
	// CountryNSearch=By.xpath("//*[@id=\"mat-option-4210\"]/span/ngx-mat-select-search/div/input");
	By CountryNSelOpt = By.xpath("//span[contains(text(),'Malaysia')]");
	// State
	By StateNClick = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[2]/div/div/form/div/div[1]/div[2]/div[2]/div[3]/app-master-form-field/div/mat-form-field/div/div[1]/div[3]/mat-select");
	// By
	// StateNSearch=By.xpath("//*[@id=\"mat-option-4212\"]/span/ngx-mat-select-search/div/input");
	By StateNSelOpt = By.xpath("//span[contains(text(),' Pulau Pinang ')]");
	// City
	By CityNClick = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[2]/div/div/form/div/div[1]/div[2]/div[3]/div[1]/app-master-form-field/div/mat-form-field/div/div[1]/div[3]/mat-select");
	// By
	// CityNSearch=By.xpath("//*[@id=\"mat-option-4214\"]/span/ngx-mat-select-search/div/input");
	By CityNSelOpt = By.xpath("//span[contains(text(),' Bukit Mertajam ')]");
	// Area
	By AreaNClick = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[2]/div/div/form/div/div[1]/div[2]/div[3]/div[2]/app-master-form-field/div/mat-form-field/div/div[1]/div[3]/mat-select");
	// By
	// AreaNSearch=By.xpath("//*[@id=\"mat-option-4216\"]/span/ngx-mat-select-search/div/input");
	By AreaNSelOpt = By.xpath("//span[contains(text(),'None')]");
	// Postcode
	By PostCodeNClick = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[2]/div/div/form/div/div[1]/div[2]/div[3]/div[3]/mat-form-field/div/div[1]/div[3]/input");
	// Referral Button
	By ReferralNButtonClick = By.cssSelector("mat-icon[ng-reflect-message='Add Referral']");

	// Save Button
	By SaveBUttonClick = By.cssSelector("mat-icon[ng-reflect-message='Save']");
	// Ok Button
	By OkButtonClick = By.cssSelector("mat-icon[ng-reflect-message='Yes']");

	// Tab 3 Document
	// Click on Document Tab
	By DocumentTabClick = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/mat-tab-header/div/div/div/div[3]/div");

	// Click on Upload button
	By UploadButtonClick = By.cssSelector("mat-icon[ng-reflect-message='Upload Document']");

	// Click on Choose File Button
	By ChooseFIleClick = By.xpath("/html/body/div[2]/div[2]/div/mat-dialog-container/app-uploadfile/div/div[2]/table/tr/td/input");
	// Upload
	By UploadClick1 = By.xpath("/html/body/div[2]/div[2]/div/mat-dialog-container/app-uploadfile/div/div[3]/div/button/span[1]/mat-icon");
	By Lab = By.xpath("//span[contains(text(),'Lab')]");
	By SideBarMenu = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/ng-sidebar/aside/app-sidebar/app-menu/ul/li[1]/a");
	// SideBar
	By SideBar = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/ng-sidebar/aside");
	By getmrnm = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-encounter-list/mat-drawer-container/mat-drawer-content/div[2]/table/tbody/tr[1]/td[1]");
	// BillingIcon
	By BillingMenu = By.xpath("//span[contains(text(),'Billing')]");

	// SideBar
	// By
	// SideBar=By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/ng-sidebar/aside");

	// OPBill
	By OPBill = By.cssSelector("a[ng-reflect-router-link='bill,opbill']");

	// SearchClick
	By AdvanceSearch = By.cssSelector("mat-icon[ng-reflect-message='Expand']");

	// MRN
	By MRNsearch = By.cssSelector("input[ng-reflect-name='PatienName']");

	// SearchClick
	By SearchClick = By.cssSelector("button[ng-reflect-message='Search']");

	// ServiceName
	By ServiceName = By.cssSelector("input[ng-reflect-name='serviceName']");
	By ServiceSelect = By.xpath("/html/body/div[2]/div/div/div/mat-option/span");

	// AddButton
	By AddButton = By.cssSelector("button[ng-reflect-message='Add Service']");

	By addref = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-registration/div[2]/mat-tab-group/div/mat-tab-body[2]/div/div/form/div/div[1]/div[2]/div[1]/div[2]/button/span[1]/mat-icon");

	// ServiceName2
	By ServiceName2 = By.cssSelector("input[ng-reflect-name='serviceName']");
	By ServiceSelect2 = By.xpath("//span[contains(text(),'MRI Brain')]");

	// SaveButton
	By SaveButton1 = By.cssSelector("mat-icon[ng-reflect-message='Save']");

	// Confirm
	By Confirm1 = By.cssSelector("mat-icon[ng-reflect-message='Yes']");

	// FinalBill
	By FinalBill = By.cssSelector("mat-checkbox[ng-reflect-id='Final Bill']");

	// SaveButton
	By SaveButton = By.cssSelector("mat-icon[ng-reflect-message='Save']");

	// Confirm
	By Confirm = By.cssSelector("mat-icon[ng-reflect-message='Yes']");

	By edit = By.cssSelector("mat-icon[ng-reflect-message='Edit']");
	By Click_on_arrow_Growth = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-incubation-details/div[3]/mat-table/mat-row[1]/mat-cell[2]/mat-form-field/div/div[1]/div[3]/mat-select/div/div[2]");
	By select_status_nogrwoth = By.xpath("//mat-select//span[contains(text(),'No Growth')]");
	By Click_on_2arrow_Growth = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-incubation-details/div[3]/mat-table/mat-row[1]/mat-cell[3]/mat-form-field/div/div[1]/div[3]/mat-select/div/div[2]");
	By select_2status_nogrwoth = By.xpath("//mat-option//span[contains(text(),'Pending')]");
	By Culture = By.cssSelector("mat-select[ng-reflect-placeholder='Add Culture']");
	By CultureNoGrowth = By.xpath("//mat-option//span[contains(text(),'No Growth')]");
	By Staus = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-incubation-details/div[4]/mat-grid-list/div/mat-grid-tile[2]/div/mat-form-field/div/div[1]/div[3]/mat-select/div/div[2]");
	By select_status = By.xpath("//mat-option//span[contains(text(),'Pending')]");
	By Save_sample = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-incubation-details/div[4]/mat-grid-list/div/mat-grid-tile[4]/div/button[3]");
	By yes = By.cssSelector("mat-icon[ng-reflect-message='Yes']");
	By Finalstaus = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-incubation/div[3]/mat-table/mat-row/mat-cell[13]");
	//Confirm
	//By Confirm=By.cssSelector("mat-icon[ng-reflect-message='Yes']");

	//PaymentAdd
	By PaymentAdd=By.cssSelector("mat-icon[ng-reflect-message='Add Payment Details']");

	//ReceivedFrom
	By ReceivedFrom=By.cssSelector("input[ng-reflect-name='receivedFrom']");

	//IDNo
	By IdentificatioNo=By.cssSelector("input[ng-reflect-name='indentificationNumber']");

	//Remarks 
	By Remarks=By.cssSelector("input[ng-reflect-placeholder='Remark']");

	//Save
	By SaveButton2=By.cssSelector("mat-icon[ng-reflect-message='Save Payment Details']");

	//Confirm2
	By Confirm2=By.cssSelector("mat-icon[ng-reflect-message='Yes']");

	By checkboxcricle = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-opdbill/app-patient-search/div/div/div/table/tbody/tr[1]/td[1]/div/mat-radio-button/label/span[1]/span[1]");	

	By selectreco = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-phlebotomy/mat-tab-group/div/mat-tab-body[1]/div/div/mat-table/mat-row/mat-cell[1]");
	By selectrecheckbox = By.xpath("//table/tr[1]/td[1]/mat-checkbox/label/span[1]");
	By Printandsample = By.cssSelector("img[title='Print and Submit']");
	By saveandsubmit = By.cssSelector("mat-icon[ng-reflect-message='Print and Submit']");
	By centerreceving = By.id("22664");
	By centerrecivecwhck = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-central-receiving/div[3]/mat-table/mat-row/mat-cell[1]/mat-checkbox");
	By receiversubmit = By.cssSelector("button[ng-reflect-message='Receive Sample']");
	By receivesample = By.cssSelector("mat-icon[ng-reflect-message='Receive Sample']");
	By Pathology = By.id("22663");
	By sampleprocessing = By.cssSelector("a[ng-reflect-router-link='pathology,sample-processing']");
	By checkclcik = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-sample-processing/div[3]/mat-table/mat-row/mat-cell[1]/mat-checkbox/label/span[1]");
	By Acceptsample = By.cssSelector("button[ng-reflect-message='Accept Sample']");
	By Acceptsamplesave = By.cssSelector("mat-icon[ng-reflect-message='Accept Sample']");
	By SideBarr=By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/ng-sidebar/aside");
	By Growth = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-incubation-details/div[3]/mat-table/mat-row[1]/mat-cell[2]/mat-form-field/div/div[1]/div[3]/mat-select");
	By Growth12 = By.xpath("//mat-option//span[contains(text(),'Growth')]");
	By status = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-incubation-details/div[3]/mat-table/mat-row[1]/mat-cell[3]/mat-form-field/div/div[1]/div[3]/mat-select");
	By stats2 = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-incubation-details/div[3]/mat-table/mat-row[2]/mat-cell[3]/mat-form-field/div/div[1]/div[3]/mat-select");
	By Completed = By.xpath("//mat-option//span[contains(text(),'Completed')]");
	By cultur = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-incubation-details/div[4]/mat-grid-list/div/mat-grid-tile[1]/div/mat-form-field/div/div[1]/div[3]/mat-select");
	By status2  = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-incubation-details/div[4]/mat-grid-list/div/mat-grid-tile[2]/div/mat-form-field/div/div[1]/div[3]/mat-select");
	By savesample = By.cssSelector("img[title='Save Sample']");
	By Yess = By.cssSelector("mat-icon[ng-reflect-message='Yes']");
	By growthrwosec = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-incubation-details/div[3]/mat-table/mat-row[2]/mat-cell[2]/mat-form-field/div/div[1]/div[3]/mat-select");
	By Observation = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-micro-result-entrydetails/mat-tab-group/div/mat-tab-body[1]/div/div/div[1]/div[3]/mat-grid-list/div/mat-grid-tile[1]/div/mat-form-field/div/div[1]/div[3]/mat-select");
	By pushcell = By.xpath("//span[contains(text(),' Pus cells ')]");
	By response = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-micro-result-entrydetails/mat-tab-group/div/mat-tab-body[1]/div/div/div[1]/div[3]/mat-grid-list/div/mat-grid-tile[2]/div/mat-form-field/div/div[1]/div[3]/mat-select");
	By formula = By.xpath("//span[contains(text(),' ≥ 25/lpf ')]");
	By addformula = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-micro-result-entrydetails/mat-tab-group/div/mat-tab-body[1]/div/div/div[1]/div[3]/mat-grid-list/div/mat-grid-tile[3]/div/button/span[1]/mat-icon");
	By Identificationtab = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-micro-result-entrydetails/mat-tab-group/div/mat-tab-body[1]/div/div/div[2]/mat-tab-group/mat-tab-header/div/div/div/div[2]");
	By organism = By.cssSelector("mat-select[ng-reflect-placeholder='Organism']");
	By input = By.cssSelector("input[placeholder='Search']");
	By Panel = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-micro-result-entrydetails/mat-tab-group/div/mat-tab-body[1]/div/div/div[2]/mat-tab-group/div/mat-tab-body[2]/div/div[1]/mat-table/mat-row/mat-cell[2]/mat-form-field/div/div[1]/div[3]/mat-select");
	By des_3 = By.xpath("//span[contains(text(),'Organism Panel01')]");  
	By method = By.cssSelector("mat-select[ng-reflect-placeholder='method']");
	By cultureandsensitive = By.xpath("//span[contains(text(),'Culture & Sensitive')]");
	By cafezai = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-micro-result-entrydetails/mat-tab-group/div/mat-tab-body[1]/div/div/div[2]/mat-tab-group/div/mat-tab-body[2]/div/mat-tab-group/div/mat-tab-body/div/div/mat-table/mat-row[1]/td[2]/input");
	By Interpetation = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-micro-result-entrydetails/mat-tab-group/div/mat-tab-body[1]/div/div/div[2]/mat-tab-group/div/mat-tab-body[2]/div/mat-tab-group/div/mat-tab-body/div/div/mat-table/mat-row/mat-cell/mat-form-field/div/div[1]/div[3]/mat-select");
	By savesampleidenti = By.cssSelector("img[title='Submit Sample']");
	By resultentry = By.id("15678");
	By interpretopt = By.xpath("//span[contains(text(),'Susceptible')]"); 
	By resultentydetailseidt = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-micro-result-entry/div[3]/mat-table/mat-row[1]/mat-cell[15]/mat-icon");
	By getmrnre = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-phlebotomy/mat-tab-group/div/mat-tab-body[1]/div/div/mat-table/mat-row/mat-cell[1]");
	By Sythaphil = By.xpath("//span[contains(text(),' Staphylococci ')]");
	By descopt = By.xpath("/html/body/div[2]/div[2]/div/div/div/mat-option[2]/span");
	By getmrnober = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-incubation/div[3]/mat-table/mat-row[1]/mat-cell[1]");
	By getmrn2 = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-micro-result-entry/div[3]/mat-table/mat-row[1]/mat-cell[2]");
	By Resultcerification = By.cssSelector("a[ng-reflect-router-link='microbiology,result-entry-vari']"); 
	By approve = By.id("15680"); 
	By editverifi = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-mirco-result-verification/app-micro-result-entry/div[3]/mat-table/mat-row[1]/mat-cell[15]/mat-icon");
	By savbunitrresult = By.cssSelector("img[title='Submit Sample']");
	By verifytheststus = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-mirco-result-verification/app-micro-result-entry/div[3]/mat-table/mat-row[1]/mat-cell[14]");
	By BulkEntry = By.cssSelector("img[title='Bulk Entry']");
	By opd2 = By.xpath("/html/body/div[2]/div[2]/div/div/div/div/div[1]/div[1]/div[1]/div/img");
	By EncounterListw=By.xpath("//span[contains(text(),'Encounter List')]");
	By getpataientname = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-patient-encounter-list/mat-drawer-container/mat-drawer-content/div[2]/table/tbody/tr[1]/td[1]");
	By name = By.cssSelector("input[ng-reflect-placeholder='Patient MRN']");
	By Checkboixfdd = By.xpath("//div[3]/mat-table/mat-row[1]/mat-cell[1]/mat-checkbox");
	By Getmrn = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-micro-result-entry/div[3]/mat-table/mat-row[1]/mat-cell[2]");
	public enter_media_observation_identificationsave(WebDriver driver)
	{
		this.driver=driver;
	}
	By OPD=By.xpath("//span[contains(text(),'OPD')]");
	By OPD3=By.xpath("/html/body/div[2]/div[2]/div/div/div/div/div[1]/div[1]/div[1]/div/img");
	By Result_entryupbuton = By.xpath("//mat-row[1]/mat-cell[5]");
	By Result_entryupbuton6 = By.xpath("//mat-row[1]/mat-cell[6]");
	By selctreco = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-result-entry-processing/div[3]/div[2]/mat-table/mat-row/mat-cell[1]");

	By ResultEntry1 = By.cssSelector("2266302");
	By ThreeDot = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-result-entry/div[3]/mat-table/mat-row[1]/mat-cell[15]/mat-icon");
	By UplaodDocument = By.xpath("/html/body/div[5]/div[2]/div/div/div/button[2]/span");
	By UploadDoc2= By.cssSelector("mat-icon[ng-reflect-message='Upload Document']");
	By ChooseFile = By.xpath("/html/body/div[5]/div[4]/div/mat-dialog-container/app-uploadfile/div/div[2]/table/tr/td/input");
	By UplaodBuuton3 = By.xpath("/html/body/div[5]/div[4]/div/mat-dialog-container/app-uploadfile/div/div[3]/div/button/span[1]/mat-icon");
	By Save = By.cssSelector("mat-icon[ng-reflect-message='Save']");

	By ResultValidation = By.id("2266303");

	By GetMrn = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-verification-list/app-result-entry/div[3]/mat-table/mat-row[1]/mat-cell[2]");

	By SelectCheckbox = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-result-entry/div[3]/mat-table/mat-row[1]/mat-cell[1]/mat-checkbox");
	By Testvale01 = By.xpath("//tr[1]/td[2]/div/input");
	By Testvale02 = By.xpath("//tr[2]/td[2]/div/input");
	By Testvale03 = By.xpath("//tr[3]/td[2]/div/input");
	By Testvale04 = By.xpath("//tr[4]/td[2]/div/input");
	By Testvale05 = By.xpath("//tr[5]/td[2]/div/input");
	By Testvale06 = By.xpath("//tr[6]/td[2]/div/input");
	By Testvale07 = By.xpath("//tr[7]/td[2]/div/input");
	By Testvale08 = By.xpath("//tr[8]/td[2]/div/input");
	By Testvale09 = By.xpath("//tr[9]/td[2]/div/input");
	By Testvale10 = By.xpath("//tr[10]/td[2]/div/input");
	By Testvale11 = By.xpath("//tr[11]/td[2]/div/input");
	By Testvale12 = By.xpath("//tr[12]/td[2]/div/input");

	By save1 = By.cssSelector("img[title='Save']");
	By Restest = By.cssSelector("img[title='Retest']");
	
	By SelectRec = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-result-entry-processing/div[3]/div[2]/mat-table/mat-row/mat-cell[1]");
	
	By Validation = By.id("2266303");
	By Reject = By.cssSelector("img[title='Reject']");
	By RejectReason= By.xpath("/html/body/div[5]/div[2]/div/mat-dialog-container/app-sample-processing-dialog/div/div/form/div[1]/mat-form-field/div/div[1]/div[3]/mat-select/div/div[1]");

	By rejectoptt = By.xpath("//span[contains(text(),'Sample Reject')]");
	By CheckBoxx = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-verification-list/app-result-entry/div[3]/mat-table/mat-row[1]/mat-cell[1]/mat-checkbox/label/span[1]");
    By rejectsave = By.cssSelector("mat-icon[ng-reflect-message='Reject']");
	By checkbox2 = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-result-entry-processing/div[3]/div[2]/mat-table/mat-row/mat-cell[1]/mat-checkbox/label/span[1]");
	
	By checkboxresultentrt = By.xpath("/html/body/app-root/app-layout/ng-sidebar-container/div/div/mat-drawer-container/mat-drawer-content/div/div/app-result-entry-processing/div[3]/div[2]/mat-table/mat-row/mat-cell[1]/mat-checkbox/label/span[1]");
	By Resultentrysave = By.cssSelector("img[title='Save']");
	By resultentrysave = By.cssSelector("img[title='Submit']");
	
	By GetSampnoresutvali = By.xpath("//app-result-entry/div[3]/mat-table/mat-row[1]/mat-cell[6]");
	By resultenrtyvalisamono = By.cssSelector("input[ng-reflect-placeholder='Sample No']");
	By SercahButton = By.cssSelector("button[ng-reflect-message='Search']");
	By Abnormal = By.cssSelector("mat-icon[ng-reflect-message='Abnormal']");
	By Normal = By.cssSelector("mat-icon[ng-reflect-message='Normal']");
	By Critical = By.cssSelector("mat-icon[ng-reflect-message='Critical']");
	By name_3 = By.cssSelector("input[ng-reflect-placeholder='Patient Name']");
	


	

	
	public void jj() throws InterruptedException {



		driver.findElement(mainmenu).click();
		Thread.sleep(3000);

		driver.findElement(Lab).click();
		Thread.sleep(3000);

		driver.findElement(Expand).click();
		Thread.sleep(3000);

		driver.findElement(name).sendKeys(takename);
		Thread.sleep(3000);

		driver.findElement(sercahghh).click();
		Thread.sleep(3000);

		driver.findElement(selectreco).click();
		Thread.sleep(3000);

		driver.findElement(selectrecheckbox).click();
		Thread.sleep(3000);

		driver.findElement(Printandsample).click();
		Thread.sleep(3000);

		driver.findElement(saveandsubmit).click();
		Thread.sleep(3000);

		String windowHandle6 = driver.getWindowHandle();
		ArrayList<Object> tabs2u = new ArrayList<Object>(driver.getWindowHandles());
		driver.switchTo().window((String) tabs2u.get(0));
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(centerreceving).click();
		Thread.sleep(3000);

		driver.findElement(centerrecivecwhck).click();
		Thread.sleep(3000);

		driver.findElement(receiversubmit).click();
		Thread.sleep(3000);

		driver.findElement(receivesample).click();
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(Pathology).click();
		Thread.sleep(3000);

		driver.findElement(sampleprocessing).click();
		Thread.sleep(3000);

		driver.findElement(checkclcik).click();
		Thread.sleep(3000);

		driver.findElement(Acceptsample).click();
		Thread.sleep(3000);

		driver.findElement(Acceptsamplesave).click();
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(Microbilogy).click();
		Thread.sleep(3000);

		driver.findElement(MediaObservation).click();
		Thread.sleep(3000);

		driver.findElement(clickonedit).click();
		Thread.sleep(3000);

		driver.findElement(Growth).click();
		Thread.sleep(3000);

		driver.findElement(Growth12).click();
		Thread.sleep(3000);

		driver.findElement(status).click();
		Thread.sleep(3000);

		driver.findElement(Completed).click();
		Thread.sleep(3000);

		driver.findElement(growthrwosec).click();
		Thread.sleep(3000);

		driver.findElement(Growth12).click();
		Thread.sleep(3000);

		driver.findElement(stats2).click();
		Thread.sleep(3000);

		driver.findElement(Completed).click();
		Thread.sleep(3000);

		driver.findElement(cultur).click();
		Thread.sleep(3000);

		driver.findElement(Growth12).click();
		Thread.sleep(3000);

		driver.findElement(status2).click();
		Thread.sleep(3000);

		driver.findElement(Completed).click();
		Thread.sleep(3000);

		driver.findElement(savesample).click();
		Thread.sleep(3000);

		driver.findElement(Yess).click();
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(Microbilogy).click();
		Thread.sleep(3000);

		driver.findElement(resultentry).click();
		Thread.sleep(3000);

		driver.findElement(resultentydetailseidt).click();
		Thread.sleep(3000);

		driver.findElement(SaveResultentry).click();
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(DiscardSample).click();
		Thread.sleep(3000);

		driver.findElement(Clickonchecbodsx).click();
		Thread.sleep(3000);

		driver.findElement(MarkDiscard).click();
		Thread.sleep(3000);

		driver.findElement(DiscardType).click();
		Thread.sleep(3000);

		driver.findElement(Discatrtyoeopt).click();
		Thread.sleep(3000);

		driver.findElement(Discardon).click();
		Thread.sleep(3000);

		driver.findElement(doscardsave).click();
		Thread.sleep(3000);

		String IssueListindvrify = driver.findElement(DiscardSamplem).getText();
		String ActualQuantity = IssueListindvrify;
		String ExpectedQuantity = IssueListindvrify;
		Thread.sleep(3000);

		driver.findElement(DiscardSamplem).click();
		Thread.sleep(3000);


	}


	public void Pending_for_Discard_Discard_Sample() throws InterruptedException {

		Thread.sleep(8000); 

		driver.findElement(mainmenu).click();
		Thread.sleep(3000);

		driver.findElement(Lab).click();
		Thread.sleep(3000);

		driver.findElement(Expand).click();
		Thread.sleep(3000);

		driver.findElement(name).sendKeys(takename);
		Thread.sleep(3000);

		driver.findElement(sercahghh).click();
		Thread.sleep(3000);

		driver.findElement(selectreco).click();
		Thread.sleep(3000);

		driver.findElement(selectrecheckbox).click();
		Thread.sleep(3000);

		driver.findElement(Printandsample).click();
		Thread.sleep(3000);

		driver.findElement(saveandsubmit).click();
		Thread.sleep(3000);

		String windowHandle6 = driver.getWindowHandle();
		ArrayList<Object> tabs2u = new ArrayList<Object>(driver.getWindowHandles());
		driver.switchTo().window((String) tabs2u.get(0));
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(centerreceving).click();
		Thread.sleep(3000);

		driver.findElement(centerrecivecwhck).click();
		Thread.sleep(3000);

		driver.findElement(receiversubmit).click();
		Thread.sleep(3000);

		driver.findElement(receivesample).click();
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(Pathology).click();
		Thread.sleep(3000);

		driver.findElement(sampleprocessing).click();
		Thread.sleep(3000);

		driver.findElement(checkclcik).click();
		Thread.sleep(3000);

		driver.findElement(Acceptsample).click();
		Thread.sleep(3000);

		driver.findElement(Acceptsamplesave).click();
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(Microbilogy).click();
		Thread.sleep(3000);

		driver.findElement(MediaObservation).click();
		Thread.sleep(3000);

		driver.findElement(clickonedit).click();
		Thread.sleep(3000);

		driver.findElement(Growth).click();
		Thread.sleep(3000);

		driver.findElement(Growth12).click();
		Thread.sleep(3000);

		driver.findElement(status).click();
		Thread.sleep(3000);

		driver.findElement(Completed).click();
		Thread.sleep(3000);

		driver.findElement(growthrwosec).click();
		Thread.sleep(3000);

		driver.findElement(Growth12).click();
		Thread.sleep(3000);

		driver.findElement(stats2).click();
		Thread.sleep(3000);

		driver.findElement(Completed).click();
		Thread.sleep(3000);

		driver.findElement(cultur).click();
		Thread.sleep(3000);

		driver.findElement(Growth12).click();
		Thread.sleep(3000);

		driver.findElement(status2).click();
		Thread.sleep(3000);

		driver.findElement(Completed).click();
		Thread.sleep(3000);

		driver.findElement(savesample).click();
		Thread.sleep(3000);

		driver.findElement(Yess).click();
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(Microbilogy).click();
		Thread.sleep(3000);

		driver.findElement(resultentry).click();
		Thread.sleep(3000);

		driver.findElement(resultentydetailseidt).click();
		Thread.sleep(3000);

		driver.findElement(SaveResultentry).click();
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(DiscardSample).click();
		Thread.sleep(3000);

		driver.findElement(Clickonchecbodsx).click();
		Thread.sleep(3000);

		driver.findElement(MarkDiscard).click();
		Thread.sleep(3000);

		driver.findElement(DiscardType).click();
		Thread.sleep(3000);

		driver.findElement(Discatrtyoeopt).click();
		Thread.sleep(3000);

		driver.findElement(Discardon).click();
		Thread.sleep(3000);

		driver.findElement(doscardsave).click();
		Thread.sleep(3000);

		String IssueListindvrify = driver.findElement(DiscardSamplem).getText();
		String ActualQuantity = IssueListindvrify;
		String ExpectedQuantity = IssueListindvrify;
		Thread.sleep(3000);

		driver.findElement(DiscardSamplem).click();
		Thread.sleep(3000);


	}

	public void Result_Entry_View_orders() throws InterruptedException {

		Thread.sleep(10000);

		

		// Navigating to the LAB module
		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();
		Thread.sleep(12000);

		// Expanding and searching for a patient
		driver.findElement(Expand).click();
		Thread.sleep(1000);
		driver.findElement(name_3).sendKeys("HISCKLMRN24817978");
		Thread.sleep(1000);
		driver.findElement(sercahghh).click();
		Thread.sleep(3000);

		// Selecting a record and processing the sample
		driver.findElement(selectreco).click();
		Thread.sleep(1000);
		driver.findElement(selectrecheckbox).click();
		Thread.sleep(2000);
		driver.findElement(Printandsample).click();
		Thread.sleep(2000);
		driver.findElement(saveandsubmit).click();
		// driver.findElement(yes).click();
		Thread.sleep(2000);
		// driver.findElement(saveandsubmit).click();
		Thread.sleep(12000);

		// Handling a new window/tab if opened
		String mainWindow = driver.getWindowHandle();
		ArrayList<String> tabs = new ArrayList<>(driver.getWindowHandles());
		driver.switchTo().window(tabs.get(0)); // Switch back to the original tab
		Thread.sleep(1000);

		//  driver.findElement(yes).click();


		// Navigating through the sidebar
		driver.findElement(SideBarr).click();
		Thread.sleep(1000);
		driver.findElement(centerreceving).click();
		Thread.sleep(3000);
		driver.findElement(centerrecivecwhck).click();
		Thread.sleep(1000);
		driver.findElement(receiversubmit).click();
		Thread.sleep(1000);
		driver.findElement(receivesample).click();
		Thread.sleep(3000);

		// Continuing with Pathology and Sample Processing
		driver.findElement(SideBarr).click();
		// Thread.sleep(3000);
		// driver.findElement(Pathology).click();
		Thread.sleep(3000);
		driver.findElement(sampleprocessing).click();
		Thread.sleep(3000);
		driver.findElement(checkclcik).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsample).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsamplesave).click();
		Thread.sleep(3000);

		// Microbiology and Media Observation
		driver.findElement(SideBarr).click();
		Thread.sleep(3000);
		driver.findElement(Microbilogy).click();
		Thread.sleep(3000);
		driver.findElement(MediaObservation).click();
		Thread.sleep(3000);
		driver.findElement(clickonedit).click();
		Thread.sleep(3000);

		driver.findElement(Growth).click();
		Thread.sleep(3000);
		driver.findElement(Growth12).click();
		Thread.sleep(3000);
		driver.findElement(status).click();
		Thread.sleep(3000);
		driver.findElement(Completed).click();
		Thread.sleep(3000);

		driver.findElement(cultur).click();
		Thread.sleep(3000);
		driver.findElement(Growth12).click();
		Thread.sleep(3000);
		driver.findElement(status2).click();
		Thread.sleep(3000);
		driver.findElement(Completed).click();
		Thread.sleep(3000);

		driver.findElement(savesample).click();
		Thread.sleep(3000);
		driver.findElement(Yess).click();
		Thread.sleep(3000);
		driver.findElement(SideBarr).click();
		Thread.sleep(3000);
		driver.findElement(Microbilogy).click();
		Thread.sleep(3000);

		// Result Entry Dashboard
		driver.findElement(resultentry).click();
		Thread.sleep(3000);
		String mrn = driver.findElement(getmrn2).getText();
		Thread.sleep(3000);
		driver.findElement(Expand).click();
		Thread.sleep(1000);
		driver.findElement(name_3).sendKeys(mrn);
		Thread.sleep(1000);
		driver.findElement(sercahghh).click();
		Thread.sleep(3000);
		String takemerntovaliide = driver.findElement(Getmrnvrify).getText();

		String Actaul = takemerntovaliide;
		String Expected = takemerntovaliide;

		Assert.assertEquals(Actaul, Expected, "Verify the vew order");

	}

	

	
	public void EnterCritical() throws InterruptedException {





		Thread.sleep(12000);		

		WebDriverWait wait = new WebDriverWait(driver,6000);



		/*
		Thread.sleep(1000); 

		driver.findElement(mainmenu2).click(); 
		Thread.sleep(8000);
		driver.findElement(OPD3).click(); 
		Thread.sleep(8000);

		//Thread.sleep(1000);
		driver.findElement(SideBarMenu).click();
		Thread.sleep(2000);
		driver.findElement(RegistrationMenu).click();
		Thread.sleep(2000); 
		driver.findElement(EncounterListw).click();
		Thread.sleep(3000);

		String takename = driver.findElement(getpataientname).getText();
		 */

		// Navigating to the LAB module
		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();
		Thread.sleep(10000);

		// Expanding and searching for a patient
		driver.findElement(Expand).click();
		Thread.sleep(1000);
		driver.findElement(name_3).sendKeys("HISCKLMRN24817978");
		Thread.sleep(1000);
		driver.findElement(sercahghh).click();
		Thread.sleep(3000);

		// Selecting a record and processing the sample
		driver.findElement(selectreco).click();
		Thread.sleep(1000);
		driver.findElement(selectrecheckbox).click();
		Thread.sleep(2000);
		driver.findElement(Printandsample).click();
		Thread.sleep(2000);


		driver.findElement(yes).click();

		Thread.sleep(2000);

		driver.findElement(saveandsubmit).click();
		// driver.findElement(yes).click();
		Thread.sleep(2000);
		// driver.findElement(saveandsubmit).click();
		Thread.sleep(12000);

		// Handling a new window/tab if opened
		String mainWindow = driver.getWindowHandle();
		ArrayList<String> tabs = new ArrayList<>(driver.getWindowHandles());
		driver.switchTo().window(tabs.get(0)); // Switch back to the original tab
		Thread.sleep(1000);

		//  driver.findElement(yes).click();


		// Navigating through the sidebar
		driver.findElement(SideBarr).click();
		Thread.sleep(1000);
		driver.findElement(centerreceving).click();
		Thread.sleep(3000);
		driver.findElement(centerrecivecwhck).click();
		Thread.sleep(1000);
		driver.findElement(receiversubmit).click();
		Thread.sleep(1000);
		driver.findElement(receivesample).click();
		Thread.sleep(3000);

		// Continuing with Pathology and Sample Processing
		driver.findElement(SideBarr).click();
		// Thread.sleep(3000);
		// driver.findElement(Pathology).click();
		Thread.sleep(3000);
		driver.findElement(sampleprocessing).click();
		Thread.sleep(3000);
		driver.findElement(checkclcik).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsample).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsamplesave).click();
		Thread.sleep(3000);


		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		//driver.findElement(Pathology).click();
		//Thread.sleep(3000);

		driver.findElement(ResultEntry1).click();
		Thread.sleep(3000);

		//driver.findElement(checkboocx).click();
		//Thread.sleep(3000);

		driver.findElement(Result_entryupbuton6).click();
		Thread.sleep(3000);

		//driver.findElement(InternalDemo).sendKeys("Enter Abnormal Result");
		//Thread.sleep(3000);

		driver.findElement(SelectRec).click();
		Thread.sleep(3000);

		driver.findElement(Testvale01).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale02).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale03).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale04).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale05).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale06).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale07).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale08).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale09).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale10).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale11).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale12).sendKeys("4");
		Thread.sleep(3000);
		
	
		driver.findElement(checkboxresultentrt).click();
		Thread.sleep(3000);

		driver.findElement(Resultentrysave).click();
		Thread.sleep(3000);


		driver.findElement(checkboxresultentrt).click();
		Thread.sleep(3000);

		driver.findElement(resultentrysave).click();
		Thread.sleep(3000);


		driver.findElement(SideBarr).click();
		Thread.sleep(3000);


		//driver.findElement(Pathology).click();
		//Thread.sleep(3000);

		driver.findElement(ResultValidation).click();
		Thread.sleep(3000);

		Thread.sleep(4000);

		String Enter_Sample_No = driver.findElement(GetSampnoresutvali).getText();

		Thread.sleep(3000);

		driver.findElement(Expand).click();
		Thread.sleep(3000);

		driver.findElement(resultenrtyvalisamono).sendKeys(Enter_Sample_No);
		Thread.sleep(4000);

		driver.findElement(SercahButton).click();
		Thread.sleep(3000);



		String AbnormalColorCode = driver.findElement(Critical).getText();



		String Actual = AbnormalColorCode;
		String Expected = AbnormalColorCode;

		Assert.assertEquals(Actual, Expected, "Verify Critical Result");

	}

	public void  Enter_Normal_Result() throws InterruptedException {



		Thread.sleep(12000);		

		WebDriverWait wait = new WebDriverWait(driver,6000);



		/*
		Thread.sleep(1000); 

		driver.findElement(mainmenu2).click(); 
		Thread.sleep(8000);
		driver.findElement(OPD3).click(); 
		Thread.sleep(8000);

		//Thread.sleep(1000);
		driver.findElement(SideBarMenu).click();
		Thread.sleep(2000);
		driver.findElement(RegistrationMenu).click();
		Thread.sleep(2000); 
		driver.findElement(EncounterListw).click();
		Thread.sleep(3000);

		String takename = driver.findElement(getpataientname).getText();
		 */

		// Navigating to the LAB module
		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();
		Thread.sleep(10000);

		// Expanding and searching for a patient
		driver.findElement(Expand).click();
		Thread.sleep(1000);
		driver.findElement(name_3).sendKeys("HISCKLMRN24817978");
		Thread.sleep(1000);
		driver.findElement(sercahghh).click();
		Thread.sleep(3000);

		// Selecting a record and processing the sample
		driver.findElement(selectreco).click();
		Thread.sleep(1000);
		driver.findElement(selectrecheckbox).click();
		Thread.sleep(2000);
		driver.findElement(Printandsample).click();
		Thread.sleep(2000);


		driver.findElement(yes).click();

		Thread.sleep(2000);

		driver.findElement(saveandsubmit).click();
		// driver.findElement(yes).click();
		Thread.sleep(2000);
		// driver.findElement(saveandsubmit).click();
		Thread.sleep(12000);

		// Handling a new window/tab if opened
		String mainWindow = driver.getWindowHandle();
		ArrayList<String> tabs = new ArrayList<>(driver.getWindowHandles());
		driver.switchTo().window(tabs.get(0)); // Switch back to the original tab
		Thread.sleep(1000);

		//  driver.findElement(yes).click();


		// Navigating through the sidebar
		driver.findElement(SideBarr).click();
		Thread.sleep(1000);
		driver.findElement(centerreceving).click();
		Thread.sleep(3000);
		driver.findElement(centerrecivecwhck).click();
		Thread.sleep(1000);
		driver.findElement(receiversubmit).click();
		Thread.sleep(1000);
		driver.findElement(receivesample).click();
		Thread.sleep(3000);

		// Continuing with Pathology and Sample Processing
		driver.findElement(SideBarr).click();
		// Thread.sleep(3000);
		// driver.findElement(Pathology).click();
		Thread.sleep(3000);
		driver.findElement(sampleprocessing).click();
		Thread.sleep(3000);
		driver.findElement(checkclcik).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsample).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsamplesave).click();
		Thread.sleep(3000);


		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		//driver.findElement(Pathology).click();
		//Thread.sleep(3000);

		driver.findElement(ResultEntry1).click();
		Thread.sleep(3000);

		//driver.findElement(checkboocx).click();
		//Thread.sleep(3000);

		driver.findElement(Result_entryupbuton6).click();
		Thread.sleep(3000);

		//driver.findElement(InternalDemo).sendKeys("Enter Abnormal Result");
		//Thread.sleep(3000);

		driver.findElement(SelectRec).click();
		Thread.sleep(3000);

		driver.findElement(Testvale01).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale02).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale03).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale04).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale05).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale06).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale07).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale08).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale09).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale10).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale11).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale12).sendKeys("4");
		Thread.sleep(3000);
		
	
		driver.findElement(checkboxresultentrt).click();
		Thread.sleep(3000);

		driver.findElement(Resultentrysave).click();
		Thread.sleep(3000);


		driver.findElement(checkboxresultentrt).click();
		Thread.sleep(3000);

		driver.findElement(resultentrysave).click();
		Thread.sleep(3000);


		driver.findElement(SideBarr).click();
		Thread.sleep(3000);


		//driver.findElement(Pathology).click();
		//Thread.sleep(3000);

		driver.findElement(ResultValidation).click();
		Thread.sleep(3000);

		Thread.sleep(4000);

		String Enter_Sample_No = driver.findElement(GetSampnoresutvali).getText();

		Thread.sleep(3000);

		driver.findElement(Expand).click();
		Thread.sleep(3000);

		driver.findElement(resultenrtyvalisamono).sendKeys(Enter_Sample_No);
		Thread.sleep(4000);

		driver.findElement(SercahButton).click();
		Thread.sleep(3000);



		String AbnormalColorCode = driver.findElement(Normal).getText();



		String Actual = AbnormalColorCode;
		String Expected = AbnormalColorCode;

		Assert.assertEquals(Actual, Expected, "Verify Normal Result");



	}

	public void Enter_Abnormal_Result() throws InterruptedException {


		Thread.sleep(12000);		


		// Navigating to the LAB module
		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();
		Thread.sleep(8000);

		// Expanding and searching for a patient
		driver.findElement(Expand).click();
		Thread.sleep(1000);
		driver.findElement(name_3).sendKeys("PASA");
		Thread.sleep(1000);
		driver.findElement(sercahghh).click();
		Thread.sleep(3000);

		// Selecting a record and processing the sample
		driver.findElement(selectreco).click();
		Thread.sleep(1000);
		driver.findElement(selectrecheckbox).click();
		Thread.sleep(2000);
		driver.findElement(Printandsample).click();
		Thread.sleep(2000);


		driver.findElement(yes).click();

		Thread.sleep(2000);

		driver.findElement(saveandsubmit).click();
		// driver.findElement(yes).click();
		Thread.sleep(2000);
		// driver.findElement(saveandsubmit).click();
		Thread.sleep(12000);

		// Handling a new window/tab if opened
		String mainWindow = driver.getWindowHandle();
		ArrayList<String> tabs = new ArrayList<>(driver.getWindowHandles());
		driver.switchTo().window(tabs.get(0)); // Switch back to the original tab
		Thread.sleep(1000);

		//  driver.findElement(yes).click();


		// Navigating through the sidebar
		driver.findElement(SideBarr).click();
		Thread.sleep(1000);
		driver.findElement(centerreceving).click();
		Thread.sleep(3000);
		driver.findElement(centerrecivecwhck).click();
		Thread.sleep(1000);
		driver.findElement(receiversubmit).click();
		Thread.sleep(1000);
		driver.findElement(receivesample).click();
		Thread.sleep(3000);

		// Continuing with Pathology and Sample Processing
		driver.findElement(SideBarr).click();
		// Thread.sleep(3000);
		// driver.findElement(Pathology).click();
		Thread.sleep(3000);
		driver.findElement(sampleprocessing).click();
		Thread.sleep(3000);
		driver.findElement(checkclcik).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsample).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsamplesave).click();
		Thread.sleep(3000);


		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		//driver.findElement(Pathology).click();
		//Thread.sleep(3000);

		driver.findElement(ResultEntry1).click();
		Thread.sleep(3000);

		//driver.findElement(checkboocx).click();
		//Thread.sleep(3000);

		driver.findElement(Result_entryupbuton6).click();
		Thread.sleep(3000);

		//driver.findElement(InternalDemo).sendKeys("Enter Abnormal Result");
		//Thread.sleep(3000);

		driver.findElement(SelectRec).click();
		Thread.sleep(3000);

		driver.findElement(Testvale01).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale02).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale03).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale04).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale05).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale06).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale07).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale08).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale09).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale10).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale11).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale12).sendKeys("4");
		Thread.sleep(3000);
		
	
		driver.findElement(checkboxresultentrt).click();
		Thread.sleep(3000);

		driver.findElement(Resultentrysave).click();
		Thread.sleep(3000);


		driver.findElement(checkboxresultentrt).click();
		Thread.sleep(3000);

		driver.findElement(resultentrysave).click();
		Thread.sleep(3000);


		driver.findElement(SideBarr).click();
		Thread.sleep(3000);


		//driver.findElement(Pathology).click();
		//Thread.sleep(3000);

		driver.findElement(ResultValidation).click();
		Thread.sleep(3000);

		Thread.sleep(4000);

		String Enter_Sample_No = driver.findElement(GetSampnoresutvali).getText();

		Thread.sleep(3000);

		driver.findElement(Expand).click();
		Thread.sleep(3000);

		driver.findElement(resultenrtyvalisamono).sendKeys(Enter_Sample_No);
		Thread.sleep(4000);

		driver.findElement(SercahButton).click();
		Thread.sleep(3000);



		String AbnormalColorCode = driver.findElement(Abnormal).getText();



		String Actual = AbnormalColorCode;
		String Expected = AbnormalColorCode;

		Assert.assertEquals(Actual, Expected, "Verify Abnormal Result");


	}

	public void Enter_Normal_ResultModify() throws InterruptedException {



		Thread.sleep(12000);		

		WebDriverWait wait = new WebDriverWait(driver,6000);



		/*
		Thread.sleep(1000); 

		driver.findElement(mainmenu2).click(); 
		Thread.sleep(8000);
		driver.findElement(OPD3).click(); 
		Thread.sleep(8000);

		//Thread.sleep(1000);
		driver.findElement(SideBarMenu).click();
		Thread.sleep(2000);
		driver.findElement(RegistrationMenu).click();
		Thread.sleep(2000); 
		driver.findElement(EncounterListw).click();
		Thread.sleep(3000);

		String takename = driver.findElement(getpataientname).getText();
		 */

		// Navigating to the LAB module
		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();
		Thread.sleep(10000);

		// Expanding and searching for a patient
		driver.findElement(Expand).click();
		Thread.sleep(1000);
		driver.findElement(name_3).sendKeys("HISCKLMRN24817978");
		Thread.sleep(1000);
		driver.findElement(sercahghh).click();
		Thread.sleep(3000);

		// Selecting a record and processing the sample
		driver.findElement(selectreco).click();
		Thread.sleep(1000);
		driver.findElement(selectrecheckbox).click();
		Thread.sleep(2000);
		driver.findElement(Printandsample).click();
		Thread.sleep(2000);


		driver.findElement(yes).click();

		Thread.sleep(2000);

		driver.findElement(saveandsubmit).click();
		// driver.findElement(yes).click();
		Thread.sleep(2000);
		// driver.findElement(saveandsubmit).click();
		Thread.sleep(12000);

		// Handling a new window/tab if opened
		String mainWindow = driver.getWindowHandle();
		ArrayList<String> tabs = new ArrayList<>(driver.getWindowHandles());
		driver.switchTo().window(tabs.get(0)); // Switch back to the original tab
		Thread.sleep(1000);

		//  driver.findElement(yes).click();


		// Navigating through the sidebar
		driver.findElement(SideBarr).click();
		Thread.sleep(1000);
		driver.findElement(centerreceving).click();
		Thread.sleep(3000);
		driver.findElement(centerrecivecwhck).click();
		Thread.sleep(1000);
		driver.findElement(receiversubmit).click();
		Thread.sleep(1000);
		driver.findElement(receivesample).click();
		Thread.sleep(3000);

		// Continuing with Pathology and Sample Processing
		driver.findElement(SideBarr).click();
		// Thread.sleep(3000);
		// driver.findElement(Pathology).click();
		Thread.sleep(3000);
		driver.findElement(sampleprocessing).click();
		Thread.sleep(3000);
		driver.findElement(checkclcik).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsample).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsamplesave).click();
		Thread.sleep(3000);


		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		//driver.findElement(Pathology).click();
		//Thread.sleep(3000);

		driver.findElement(ResultEntry1).click();
		Thread.sleep(3000);

		//driver.findElement(checkboocx).click();
		//Thread.sleep(3000);

		driver.findElement(Result_entryupbuton6).click();
		Thread.sleep(3000);

		//driver.findElement(InternalDemo).sendKeys("Enter Abnormal Result");
		//Thread.sleep(3000);

		driver.findElement(SelectRec).click();
		Thread.sleep(3000);

		driver.findElement(Testvale01).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale02).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale03).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale04).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale05).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale06).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale07).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale08).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale09).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale10).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale11).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale12).sendKeys("4");
		Thread.sleep(3000);
		
	
		driver.findElement(checkboxresultentrt).click();
		Thread.sleep(3000);

		driver.findElement(Resultentrysave).click();
		Thread.sleep(3000);


		driver.findElement(checkboxresultentrt).click();
		Thread.sleep(3000);

		driver.findElement(resultentrysave).click();
		Thread.sleep(3000);


		driver.findElement(SideBarr).click();
		Thread.sleep(3000);


		//driver.findElement(Pathology).click();
		//Thread.sleep(3000);

		driver.findElement(ResultValidation).click();
		Thread.sleep(3000);

		Thread.sleep(4000);

		String Enter_Sample_No = driver.findElement(GetSampnoresutvali).getText();

		Thread.sleep(3000);

		driver.findElement(Expand).click();
		Thread.sleep(3000);

		driver.findElement(resultenrtyvalisamono).sendKeys(Enter_Sample_No);
		Thread.sleep(4000);

		driver.findElement(SercahButton).click();
		Thread.sleep(3000);



		String AbnormalColorCode = driver.findElement(Normal).getText();



		String Actual = AbnormalColorCode;
		String Expected = AbnormalColorCode;

		Assert.assertEquals(Actual, Expected, "Verify Normal Result");





	}
	public void EnterCrtitcale_ResultModify() throws InterruptedException {



		Thread.sleep(12000);		

		// Navigating to the LAB module
		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();
		Thread.sleep(10000);

		// Expanding and searching for a patient
		driver.findElement(Expand).click();
		Thread.sleep(1000);
		driver.findElement(name_3).sendKeys("");
		Thread.sleep(1000);
		driver.findElement(sercahghh).click();
		Thread.sleep(3000);

		// Selecting a record and processing the sample
		driver.findElement(selectreco).click();
		Thread.sleep(1000);
		driver.findElement(selectrecheckbox).click();
		Thread.sleep(2000);
		driver.findElement(Printandsample).click();
		Thread.sleep(2000);


		driver.findElement(yes).click();

		Thread.sleep(2000);

		driver.findElement(saveandsubmit).click();
		// driver.findElement(yes).click();
		Thread.sleep(2000);
		// driver.findElement(saveandsubmit).click();
		Thread.sleep(12000);

		// Handling a new window/tab if opened
		String mainWindow = driver.getWindowHandle();
		ArrayList<String> tabs = new ArrayList<>(driver.getWindowHandles());
		driver.switchTo().window(tabs.get(0)); // Switch back to the original tab
		Thread.sleep(1000);

		//  driver.findElement(yes).click();


		// Navigating through the sidebar
		driver.findElement(SideBarr).click();
		Thread.sleep(1000);
		driver.findElement(centerreceving).click();
		Thread.sleep(3000);
		driver.findElement(centerrecivecwhck).click();
		Thread.sleep(1000);
		driver.findElement(receiversubmit).click();
		Thread.sleep(1000);
		driver.findElement(receivesample).click();
		Thread.sleep(3000);

		// Continuing with Pathology and Sample Processing
		driver.findElement(SideBarr).click();
		// Thread.sleep(3000);
		// driver.findElement(Pathology).click();
		Thread.sleep(3000);
		driver.findElement(sampleprocessing).click();
		Thread.sleep(3000);
		driver.findElement(checkclcik).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsample).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsamplesave).click();
		Thread.sleep(3000);


		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		//driver.findElement(Pathology).click();
		//Thread.sleep(3000);

		driver.findElement(ResultEntry1).click();
		Thread.sleep(3000);

		//driver.findElement(checkboocx).click();
		//Thread.sleep(3000);

		driver.findElement(Result_entryupbuton6).click();
		Thread.sleep(3000);

		//driver.findElement(InternalDemo).sendKeys("Enter Abnormal Result");
		//Thread.sleep(3000);

		driver.findElement(SelectRec).click();
		Thread.sleep(3000);

		driver.findElement(Testvale01).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale02).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale03).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale04).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale05).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale06).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale07).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale08).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale09).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale10).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale11).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale12).sendKeys("4");
		Thread.sleep(3000);
		
	
		driver.findElement(checkboxresultentrt).click();
		Thread.sleep(3000);

		driver.findElement(Resultentrysave).click();
		Thread.sleep(3000);


		driver.findElement(checkboxresultentrt).click();
		Thread.sleep(3000);

		driver.findElement(resultentrysave).click();
		Thread.sleep(3000);


		driver.findElement(SideBarr).click();
		Thread.sleep(3000);


		//driver.findElement(Pathology).click();
		//Thread.sleep(3000);

		driver.findElement(ResultValidation).click();
		Thread.sleep(3000);

		Thread.sleep(4000);

		String Enter_Sample_No = driver.findElement(GetSampnoresutvali).getText();

		Thread.sleep(3000);

		driver.findElement(Expand).click();
		Thread.sleep(3000);

		driver.findElement(resultenrtyvalisamono).sendKeys(Enter_Sample_No);
		Thread.sleep(4000);

		driver.findElement(SercahButton).click();
		Thread.sleep(3000);



		String AbnormalColorCode = driver.findElement(Critical).getText();



		String Actual = AbnormalColorCode;
		String Expected = AbnormalColorCode;

		Assert.assertEquals(Actual, Expected, "Verify Critical Result");




	}
	
	public void Click_back_Verify_color_code_resultvatliaftion() throws InterruptedException {




		Thread.sleep(12000);		

		WebDriverWait wait = new WebDriverWait(driver,6000);



		/*
		Thread.sleep(1000); 

		driver.findElement(mainmenu2).click(); 
		Thread.sleep(8000);
		driver.findElement(OPD3).click(); 
		Thread.sleep(8000);

		//Thread.sleep(1000);
		driver.findElement(SideBarMenu).click();
		Thread.sleep(2000);
		driver.findElement(RegistrationMenu).click();
		Thread.sleep(2000); 
		driver.findElement(EncounterListw).click();
		Thread.sleep(3000);

		String takename = driver.findElement(getpataientname).getText();
		 */

		// Navigating to the LAB module
		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();
		Thread.sleep(10000);

		// Expanding and searching for a patient
		driver.findElement(Expand).click();
		Thread.sleep(1000);
		driver.findElement(name_3).sendKeys("HISCKLMRN24817978");
		Thread.sleep(1000);
		driver.findElement(sercahghh).click();
		Thread.sleep(3000);

		// Selecting a record and processing the sample
		driver.findElement(selectreco).click();
		Thread.sleep(1000);
		driver.findElement(selectrecheckbox).click();
		Thread.sleep(2000);
		driver.findElement(Printandsample).click();
		Thread.sleep(2000);


		driver.findElement(yes).click();

		Thread.sleep(2000);

		driver.findElement(saveandsubmit).click();
		// driver.findElement(yes).click();
		Thread.sleep(2000);
		// driver.findElement(saveandsubmit).click();
		Thread.sleep(12000);

		// Handling a new window/tab if opened
		String mainWindow = driver.getWindowHandle();
		ArrayList<String> tabs = new ArrayList<>(driver.getWindowHandles());
		driver.switchTo().window(tabs.get(0)); // Switch back to the original tab
		Thread.sleep(1000);

		//  driver.findElement(yes).click();


		// Navigating through the sidebar
		driver.findElement(SideBarr).click();
		Thread.sleep(1000);
		driver.findElement(centerreceving).click();
		Thread.sleep(3000);
		driver.findElement(centerrecivecwhck).click();
		Thread.sleep(1000);
		driver.findElement(receiversubmit).click();
		Thread.sleep(1000);
		driver.findElement(receivesample).click();
		Thread.sleep(3000);

		// Continuing with Pathology and Sample Processing
		driver.findElement(SideBarr).click();
		// Thread.sleep(3000);
		// driver.findElement(Pathology).click();
		Thread.sleep(3000);
		driver.findElement(sampleprocessing).click();
		Thread.sleep(3000);
		driver.findElement(checkclcik).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsample).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsamplesave).click();
		Thread.sleep(3000);


		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		//driver.findElement(Pathology).click();
		//Thread.sleep(3000);

		driver.findElement(ResultEntry1).click();
		Thread.sleep(3000);

		//driver.findElement(checkboocx).click();
		//Thread.sleep(3000);

		driver.findElement(Result_entryupbuton6).click();
		Thread.sleep(3000);

		//driver.findElement(InternalDemo).sendKeys("Enter Abnormal Result");
		//Thread.sleep(3000);

		driver.findElement(SelectRec).click();
		Thread.sleep(3000);

		driver.findElement(Testvale01).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale02).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale03).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale04).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale05).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale06).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale07).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale08).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale09).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale10).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale11).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale12).sendKeys("4");
		Thread.sleep(3000);
		
	
		driver.findElement(checkboxresultentrt).click();
		Thread.sleep(3000);

		driver.findElement(Resultentrysave).click();
		Thread.sleep(3000);


		driver.findElement(checkboxresultentrt).click();
		Thread.sleep(3000);

		driver.findElement(resultentrysave).click();
		Thread.sleep(3000);


		driver.findElement(SideBarr).click();
		Thread.sleep(3000);


		//driver.findElement(Pathology).click();
		//Thread.sleep(3000);

		driver.findElement(ResultValidation).click();
		Thread.sleep(3000);

		Thread.sleep(4000);

		String Enter_Sample_No = driver.findElement(GetSampnoresutvali).getText();

		Thread.sleep(3000);

		driver.findElement(Expand).click();
		Thread.sleep(3000);

		driver.findElement(resultenrtyvalisamono).sendKeys(Enter_Sample_No);
		Thread.sleep(4000);

		driver.findElement(SercahButton).click();
		Thread.sleep(3000);



		String AbnormalColorCode = driver.findElement(Normal).getText();



		String Actual = AbnormalColorCode;
		String Expected = AbnormalColorCode;

		Assert.assertEquals(Actual, Expected, "Verify Normal Result");

	}

	public void Click_back_Verify_color_code_resultvatliaftionAbnormal() throws InterruptedException {



		Thread.sleep(12000);		

		WebDriverWait wait = new WebDriverWait(driver,6000);



		/*
		Thread.sleep(1000); 

		driver.findElement(mainmenu2).click(); 
		Thread.sleep(8000);
		driver.findElement(OPD3).click(); 
		Thread.sleep(8000);

		//Thread.sleep(1000);
		driver.findElement(SideBarMenu).click();
		Thread.sleep(2000);
		driver.findElement(RegistrationMenu).click();
		Thread.sleep(2000); 
		driver.findElement(EncounterListw).click();
		Thread.sleep(3000);

		String takename = driver.findElement(getpataientname).getText();
		 */

		// Navigating to the LAB module
		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();
		Thread.sleep(10000);

		// Expanding and searching for a patient
		driver.findElement(Expand).click();
		Thread.sleep(1000);
		driver.findElement(name_3).sendKeys("HISCKLMRN24817978");
		Thread.sleep(1000);
		driver.findElement(sercahghh).click();
		Thread.sleep(3000);

		// Selecting a record and processing the sample
		driver.findElement(selectreco).click();
		Thread.sleep(1000);
		driver.findElement(selectrecheckbox).click();
		Thread.sleep(2000);
		driver.findElement(Printandsample).click();
		Thread.sleep(2000);


		driver.findElement(yes).click();

		Thread.sleep(2000);

		driver.findElement(saveandsubmit).click();
		// driver.findElement(yes).click();
		Thread.sleep(2000);
		// driver.findElement(saveandsubmit).click();
		Thread.sleep(12000);

		// Handling a new window/tab if opened
		String mainWindow = driver.getWindowHandle();
		ArrayList<String> tabs = new ArrayList<>(driver.getWindowHandles());
		driver.switchTo().window(tabs.get(0)); // Switch back to the original tab
		Thread.sleep(1000);

		//  driver.findElement(yes).click();


		// Navigating through the sidebar
		driver.findElement(SideBarr).click();
		Thread.sleep(1000);
		driver.findElement(centerreceving).click();
		Thread.sleep(3000);
		driver.findElement(centerrecivecwhck).click();
		Thread.sleep(1000);
		driver.findElement(receiversubmit).click();
		Thread.sleep(1000);
		driver.findElement(receivesample).click();
		Thread.sleep(3000);

		// Continuing with Pathology and Sample Processing
		driver.findElement(SideBarr).click();
		// Thread.sleep(3000);
		// driver.findElement(Pathology).click();
		Thread.sleep(3000);
		driver.findElement(sampleprocessing).click();
		Thread.sleep(3000);
		driver.findElement(checkclcik).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsample).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsamplesave).click();
		Thread.sleep(3000);


		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		//driver.findElement(Pathology).click();
		//Thread.sleep(3000);

		driver.findElement(ResultEntry1).click();
		Thread.sleep(3000);

		//driver.findElement(checkboocx).click();
		//Thread.sleep(3000);

		driver.findElement(Result_entryupbuton6).click();
		Thread.sleep(3000);

		//driver.findElement(InternalDemo).sendKeys("Enter Abnormal Result");
		//Thread.sleep(3000);

		driver.findElement(SelectRec).click();
		Thread.sleep(3000);

		driver.findElement(Testvale01).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale02).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale03).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale04).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale05).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale06).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale07).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale08).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale09).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale10).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale11).sendKeys("4");
		Thread.sleep(3000);

		driver.findElement(Testvale12).sendKeys("4");
		Thread.sleep(3000);
		
	
		driver.findElement(checkboxresultentrt).click();
		Thread.sleep(3000);

		driver.findElement(Resultentrysave).click();
		Thread.sleep(3000);


		driver.findElement(checkboxresultentrt).click();
		Thread.sleep(3000);

		driver.findElement(resultentrysave).click();
		Thread.sleep(3000);


		driver.findElement(SideBarr).click();
		Thread.sleep(3000);


		//driver.findElement(Pathology).click();
		//Thread.sleep(3000);

		driver.findElement(ResultValidation).click();
		Thread.sleep(3000);

		Thread.sleep(4000);

		String Enter_Sample_No = driver.findElement(GetSampnoresutvali).getText();

		Thread.sleep(3000);

		driver.findElement(Expand).click();
		Thread.sleep(3000);

		driver.findElement(resultenrtyvalisamono).sendKeys(Enter_Sample_No);
		Thread.sleep(4000);

		driver.findElement(SercahButton).click();
		Thread.sleep(3000);



		String AbnormalColorCode = driver.findElement(Abnormal).getText();



		String Actual = AbnormalColorCode;
		String Expected = AbnormalColorCode;

		Assert.assertEquals(Actual, Expected, "Verify Abnormal Result");


	}


	public void UploadDocument_resultentry() throws InterruptedException {



		// Navigating to the LAB module
		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();
		Thread.sleep(10000);

		// Expanding and searching for a patient
		driver.findElement(Expand).click();
		Thread.sleep(1000);
		driver.findElement(name_3).sendKeys("HISCKLMRN24817978");
		Thread.sleep(1000);
		driver.findElement(sercahghh).click();
		Thread.sleep(3000);

		// Selecting a record and processing the sample
		driver.findElement(selectreco).click();
		Thread.sleep(1000);
		driver.findElement(selectrecheckbox).click();
		Thread.sleep(2000);
		driver.findElement(Printandsample).click();
		Thread.sleep(2000);
		driver.findElement(saveandsubmit).click();
		// driver.findElement(yes).click();
		Thread.sleep(2000);
		// driver.findElement(saveandsubmit).click();
		Thread.sleep(12000);

		// Handling a new window/tab if opened
		String mainWindow = driver.getWindowHandle();
		ArrayList<String> tabs = new ArrayList<>(driver.getWindowHandles());
		driver.switchTo().window(tabs.get(0)); // Switch back to the original tab
		Thread.sleep(1000);

		//  driver.findElement(yes).click();


		// Navigating through the sidebar
		driver.findElement(SideBarr).click();
		Thread.sleep(1000);
		driver.findElement(centerreceving).click();
		Thread.sleep(3000);
		driver.findElement(centerrecivecwhck).click();
		Thread.sleep(1000);
		driver.findElement(receiversubmit).click();
		Thread.sleep(1000);
		driver.findElement(receivesample).click();
		Thread.sleep(3000);

		// Continuing with Pathology and Sample Processing
		driver.findElement(SideBarr).click();
		// Thread.sleep(3000);
		// driver.findElement(Pathology).click();
		Thread.sleep(3000);
		driver.findElement(sampleprocessing).click();
		Thread.sleep(3000);
		driver.findElement(checkclcik).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsample).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsamplesave).click();
		Thread.sleep(3000);
		Thread.sleep(2000);
		// Click on Sidebar again
		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		// Click on Result Entry
		driver.findElement(ResultEntry1).click();
		Thread.sleep(3000);

		driver.findElement(ThreeDot).click();
		Thread.sleep(3000);

		driver.findElement(UplaodDocument).click();
		Thread.sleep(3000);

		driver.findElement(UploadDoc2).click();
		Thread.sleep(3000);

		//driver.findElement(ChooseFile).click();
		//Thread.sleep(3000);


		// Click on Choose File to upload the desired file
		WebElement browse = driver.findElement(ChooseFile);
		String projectpath = System.getProperty("user.dir");
		StringSelection ss = new StringSelection(projectpath + "\\Text file\\Demo.txt");
		browse.sendKeys(projectpath + "\\Text file\\Demo.txt");
		Thread.sleep(5000);

		driver.findElement(UplaodBuuton3).click();
		Thread.sleep(3000);

		driver.findElement(Save).click();
		Thread.sleep(3000);


	}
	public void Result_validation_reject() throws InterruptedException {



		// Navigating to the LAB module
				driver.findElement(mainmenu).click();
				Thread.sleep(8000);
				driver.findElement(Lab).click();
				Thread.sleep(10000);
				// Navigating through the sidebar
				driver.findElement(SideBarr).click();
				Thread.sleep(1000);
				driver.findElement(Pathology).click();
				Thread.sleep(3000);
				driver.findElement(Validation).click();
				Thread.sleep(1000);
				driver.findElement(CheckBoxx).click();
				Thread.sleep(1000);
				
				driver.findElement(Reject).click();
				Thread.sleep(1000);
				
				driver.findElement(RejectReason).click();
				Thread.sleep(1000);
				
				driver.findElement(rejectoptt).click();
				Thread.sleep(1000);
				
				driver.findElement(rejectsave).click();
				Thread.sleep(1000);
				
				
	}

public void Resultvalidation() throws InterruptedException {





		// Click on menu icon
		driver.findElement(mainmenu).click();
		Thread.sleep(8000);

		// Click on Lab
		driver.findElement(Lab).click();
		Thread.sleep(3000);

		// Click on Sidebar
		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		// Click on Pathology
		driver.findElement(Pathology).click();
		Thread.sleep(3000);

		// Click on Result Validation
		driver.findElement(ResultValidation).click();
		Thread.sleep(3000);

		// Click on Expand
		driver.findElement(Expand).click();
		Thread.sleep(3000);

		String getmrn = driver.findElement(GetMrn).getText();

		// Enter "PATHOLOGY" in the name field
		driver.findElement(name).sendKeys(getmrn);
		Thread.sleep(3000);

		// Click on the search button
		driver.findElement(sercahghh).click();
		Thread.sleep(3000);


	}


	
	public void Retest_in_Result_Validation() throws InterruptedException {




		// Navigating to the LAB module
		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();
		Thread.sleep(10000);

		// Expanding and searching for a patient
		driver.findElement(Expand).click();
		Thread.sleep(1000);
		driver.findElement(name_3).sendKeys("HISCKLMRN24817978");
		Thread.sleep(1000);
		driver.findElement(sercahghh).click();
		Thread.sleep(3000);

		// Selecting a record and processing the sample
		driver.findElement(selectreco).click();
		Thread.sleep(1000);
		driver.findElement(selectrecheckbox).click();
		Thread.sleep(2000);
		driver.findElement(Printandsample).click();
		Thread.sleep(2000);
		driver.findElement(saveandsubmit).click();
		// driver.findElement(yes).click();
		Thread.sleep(2000);
		// driver.findElement(saveandsubmit).click();
		Thread.sleep(12000);

		// Handling a new window/tab if opened
		String mainWindow = driver.getWindowHandle();
		ArrayList<String> tabs = new ArrayList<>(driver.getWindowHandles());
		driver.switchTo().window(tabs.get(0)); // Switch back to the original tab
		Thread.sleep(1000);

		//  driver.findElement(yes).click();


		// Navigating through the sidebar
		driver.findElement(SideBarr).click();
		Thread.sleep(1000);
		driver.findElement(centerreceving).click();
		Thread.sleep(3000);
		driver.findElement(centerrecivecwhck).click();
		Thread.sleep(1000);
		driver.findElement(receiversubmit).click();
		Thread.sleep(1000);
		driver.findElement(receivesample).click();
		Thread.sleep(3000);

		// Continuing with Pathology and Sample Processing
		driver.findElement(SideBarr).click();
		// Thread.sleep(3000);
		// driver.findElement(Pathology).click();
		Thread.sleep(3000);
		driver.findElement(sampleprocessing).click();
		Thread.sleep(3000);
		driver.findElement(checkclcik).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsample).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsamplesave).click();
		Thread.sleep(3000);
		Thread.sleep(2000);



		// Click on Result Entry
		driver.findElement(ResultEntry1).click();
		Thread.sleep(3000);

		// Click on Checkbox
		driver.findElement(SelectCheckbox).click();
		Thread.sleep(3000);

		// Click on Result Entry Update Button
		driver.findElement(Result_entryupbuton6).click();
		Thread.sleep(3000);

		// Enter "Enter Normal Result" in Internal Demo field
		//driver.findElement(InternalDemo).sendKeys("Enter Normal Result");
		//Thread.sleep(3000);

		// Click on Select Record Again
		driver.findElement(selctreco).click();
		Thread.sleep(3000);

		// Enter values into parameters
		driver.findElement(Testvale01).sendKeys("4");
		Thread.sleep(1000);
		driver.findElement(Testvale02).sendKeys("4");
		Thread.sleep(1000);
		driver.findElement(Testvale03).sendKeys("4");
		Thread.sleep(1000);
		driver.findElement(Testvale04).sendKeys("4");
		Thread.sleep(1000);
		driver.findElement(Testvale05).sendKeys("4");
		Thread.sleep(1000);
		driver.findElement(Testvale06).sendKeys("4");
		Thread.sleep(1000);
		driver.findElement(Testvale07).sendKeys("4");
		Thread.sleep(1000);
		driver.findElement(Testvale08).sendKeys("4");
		Thread.sleep(1000);
		driver.findElement(Testvale09).sendKeys("4");
		Thread.sleep(1000);
		driver.findElement(Testvale10).sendKeys("4");
		Thread.sleep(1000);
		driver.findElement(Testvale11).sendKeys("4");
		Thread.sleep(1000);
		driver.findElement(Testvale12).sendKeys("4");
		Thread.sleep(1000);

		// Click on Checkbox
		driver.findElement(checkbox2).click();
		Thread.sleep(3000);

		// Click on Sidebar again
		driver.findElement(save1).click();
		Thread.sleep(3000);



	}
	
	public void Recollect_at_Approval() throws InterruptedException {



		// Navigating to the LAB module
		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();
		Thread.sleep(10000);

		// Expanding and searching for a patient
		driver.findElement(Expand).click();
		Thread.sleep(1000);
		driver.findElement(name_3).sendKeys("HISCKLMRN24817978");
		Thread.sleep(1000);
		driver.findElement(sercahghh).click();
		Thread.sleep(3000);

		// Selecting a record and processing the sample
		driver.findElement(selectreco).click();
		Thread.sleep(1000);
		driver.findElement(selectrecheckbox).click();
		Thread.sleep(2000);
		driver.findElement(Printandsample).click();
		Thread.sleep(2000);
		driver.findElement(saveandsubmit).click();
		// driver.findElement(yes).click();
		Thread.sleep(2000);
		// driver.findElement(saveandsubmit).click();
		Thread.sleep(12000);

		// Handling a new window/tab if opened
		String mainWindow = driver.getWindowHandle();
		ArrayList<String> tabs = new ArrayList<>(driver.getWindowHandles());
		driver.switchTo().window(tabs.get(0)); // Switch back to the original tab
		Thread.sleep(1000);

		//  driver.findElement(yes).click();


		// Navigating through the sidebar
		driver.findElement(SideBarr).click();
		Thread.sleep(1000);
		driver.findElement(centerreceving).click();
		Thread.sleep(3000);
		driver.findElement(centerrecivecwhck).click();
		Thread.sleep(1000);
		driver.findElement(receiversubmit).click();
		Thread.sleep(1000);
		driver.findElement(receivesample).click();
		Thread.sleep(3000);

		// Continuing with Pathology and Sample Processing
		driver.findElement(SideBarr).click();
		// Thread.sleep(3000);
		// driver.findElement(Pathology).click();
		Thread.sleep(3000);
		driver.findElement(sampleprocessing).click();
		Thread.sleep(3000);
		driver.findElement(checkclcik).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsample).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsamplesave).click();
		Thread.sleep(3000);
		Thread.sleep(2000);


		//Thread.sleep(3000);

		//		String kk = driver.findElement(getmrnre).getText();
		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(Microbilogy).click();
		Thread.sleep(3000);

		driver.findElement(MediaObservation).click();
		Thread.sleep(3000);

		driver.findElement(Result_entryupbuton).click();
		Thread.sleep(3000);

		driver.findElement(Growth).click();
		Thread.sleep(3000);

		driver.findElement(Growth12).click();
		Thread.sleep(3000);

		driver.findElement(status).click();
		Thread.sleep(3000);

		driver.findElement(Completed).click();
		Thread.sleep(3000);

		driver.findElement(growthrwosec).click();
		Thread.sleep(3000);

		driver.findElement(Growth12).click();
		Thread.sleep(3000);

		driver.findElement(stats2).click();
		Thread.sleep(3000);

		driver.findElement(Completed).click();
		Thread.sleep(3000);

		driver.findElement(cultur).click();
		Thread.sleep(3000);

		driver.findElement(Growth12).click();
		Thread.sleep(3000);

		driver.findElement(status2).click();
		Thread.sleep(3000);

		driver.findElement(Completed).click();
		Thread.sleep(3000);

		driver.findElement(savesample).click();
		Thread.sleep(3000);

		driver.findElement(Yess).click();
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(Microbilogy).click();
		Thread.sleep(3000);

		//********************result-entry-dashboard*****************//
		driver.findElement(resultentry).click();
		Thread.sleep(3000);

		// Strat fro here

		driver.findElement(Result_entryupbuton6).click();
		Thread.sleep(3000);

		driver.findElement(Observation).click();
		Thread.sleep(3000);

		driver.findElement(pushcell).click();
		Thread.sleep(3000);

		driver.findElement(response).click();
		Thread.sleep(3000);

		driver.findElement(formula).click();
		Thread.sleep(3000);

		driver.findElement(addformula).click();
		Thread.sleep(3000);

		driver.findElement(Identificationtab).click();
		Thread.sleep(3000);

		driver.findElement(organism).click();
		Thread.sleep(3000);

		driver.findElement(input).sendKeys("Staphylococci");
		Thread.sleep(3000);

		driver.findElement(Sythaphil).click();
		Thread.sleep(3000);

		driver.findElement(Panel).click();
		Thread.sleep(3000);

		driver.findElement(des_3).click();
		Thread.sleep(3000);

		driver.findElement(method).click();
		Thread.sleep(3000);

		driver.findElement(cultureandsensitive).click();
		Thread.sleep(3000);

		driver.findElement(cafezai).sendKeys("1");
		Thread.sleep(3000);

		driver.findElement(Interpetation).click();
		Thread.sleep(3000);

		driver.findElement(interpretopt).click();
		Thread.sleep(3000);

		driver.findElement(savesampleidenti).click();
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(Microbilogy).click();
		Thread.sleep(3000);

		driver.findElement(Resultcerification).click();
		Thread.sleep(3000);

		driver.findElement(Result_entryupbuton6).click();
		Thread.sleep(3000);

		driver.findElement(savbunitrresult).click();
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(Microbilogy).click();
		Thread.sleep(3000);

		driver.findElement(approve).click();
		Thread.sleep(3000);

		driver.findElement(Checkboixfdd).click();
		Thread.sleep(3000);

		driver.findElement(BulkEntry).click();
		Thread.sleep(3000);

		Thread.sleep(1000);

		//String kke = driver.findElement(Recollectegettext).getText();

		//String Actual = kke;
		//String Expected = kke;

		//Assert.assertEquals(Actual, Expected, "Verify Normal Result");

	}

	public void Recollect_Central_Receiving() throws InterruptedException {


		// Navigating to the LAB module
		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();
		Thread.sleep(10000);

		// Expanding and searching for a patient
		driver.findElement(Expand).click();
		Thread.sleep(1000);
		driver.findElement(name_3).sendKeys("HISCKLMRN24817978");
		Thread.sleep(1000);
		driver.findElement(sercahghh).click();
		Thread.sleep(3000);

		// Selecting a record and processing the sample
		driver.findElement(selectreco).click();
		Thread.sleep(1000);
		driver.findElement(selectrecheckbox).click();
		Thread.sleep(2000);
		driver.findElement(Printandsample).click();
		Thread.sleep(2000);
		driver.findElement(saveandsubmit).click();
		// driver.findElement(yes).click();
		Thread.sleep(2000);
		// driver.findElement(saveandsubmit).click();
		Thread.sleep(12000);

		// Handling a new window/tab if opened
		String mainWindow = driver.getWindowHandle();
		ArrayList<String> tabs = new ArrayList<>(driver.getWindowHandles());
		driver.switchTo().window(tabs.get(0)); // Switch back to the original tab
		Thread.sleep(1000);

		//  driver.findElement(yes).click();


		// Navigating through the sidebar
		driver.findElement(SideBarr).click();
		Thread.sleep(1000);
		driver.findElement(centerreceving).click();
		Thread.sleep(3000);
		driver.findElement(centerrecivecwhck).click();
		Thread.sleep(1000);
		driver.findElement(receiversubmit).click();
		Thread.sleep(1000);
		driver.findElement(receivesample).click();
		Thread.sleep(3000);

		// Continuing with Pathology and Sample Processing
		driver.findElement(SideBarr).click();
		// Thread.sleep(3000);
		// driver.findElement(Pathology).click();
		Thread.sleep(3000);
		driver.findElement(sampleprocessing).click();
		Thread.sleep(3000);
		driver.findElement(checkclcik).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsample).click();
		Thread.sleep(3000);
		driver.findElement(Acceptsamplesave).click();
		Thread.sleep(3000);
		Thread.sleep(2000);


		//Thread.sleep(3000);

		//		String kk = driver.findElement(getmrnre).getText();
		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(Microbilogy).click();
		Thread.sleep(3000);

		driver.findElement(MediaObservation).click();
		Thread.sleep(3000);

		driver.findElement(Result_entryupbuton).click();
		Thread.sleep(3000);

		driver.findElement(Growth).click();
		Thread.sleep(3000);

		driver.findElement(Growth12).click();
		Thread.sleep(3000);

		driver.findElement(status).click();
		Thread.sleep(3000);

		driver.findElement(Completed).click();
		Thread.sleep(3000);

		driver.findElement(growthrwosec).click();
		Thread.sleep(3000);

		driver.findElement(Growth12).click();
		Thread.sleep(3000);

		driver.findElement(stats2).click();
		Thread.sleep(3000);

		driver.findElement(Completed).click();
		Thread.sleep(3000);

		driver.findElement(cultur).click();
		Thread.sleep(3000);

		driver.findElement(Growth12).click();
		Thread.sleep(3000);

		driver.findElement(status2).click();
		Thread.sleep(3000);

		driver.findElement(Completed).click();
		Thread.sleep(3000);

		driver.findElement(savesample).click();
		Thread.sleep(3000);

		driver.findElement(Yess).click();
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(Microbilogy).click();
		Thread.sleep(3000);

		//********************result-entry-dashboard*****************//
		driver.findElement(resultentry).click();
		Thread.sleep(3000);

		// Strat fro here

		driver.findElement(Result_entryupbuton6).click();
		Thread.sleep(3000);

		driver.findElement(Observation).click();
		Thread.sleep(3000);

		driver.findElement(pushcell).click();
		Thread.sleep(3000);

		driver.findElement(response).click();
		Thread.sleep(3000);

		driver.findElement(formula).click();
		Thread.sleep(3000);

		driver.findElement(addformula).click();
		Thread.sleep(3000);

		driver.findElement(Identificationtab).click();
		Thread.sleep(3000);

		driver.findElement(organism).click();
		Thread.sleep(3000);

		driver.findElement(input).sendKeys("Staphylococci");
		Thread.sleep(3000);

		driver.findElement(Sythaphil).click();
		Thread.sleep(3000);

		driver.findElement(Panel).click();
		Thread.sleep(3000);

		driver.findElement(des_3).click();
		Thread.sleep(3000);

		driver.findElement(method).click();
		Thread.sleep(3000);

		driver.findElement(cultureandsensitive).click();
		Thread.sleep(3000);

		driver.findElement(cafezai).sendKeys("1");
		Thread.sleep(3000);

		driver.findElement(Interpetation).click();
		Thread.sleep(3000);

		driver.findElement(interpretopt).click();
		Thread.sleep(3000);

		driver.findElement(savesampleidenti).click();
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(Microbilogy).click();
		Thread.sleep(3000);

		driver.findElement(Resultcerification).click();
		Thread.sleep(3000);

		driver.findElement(Result_entryupbuton6).click();
		Thread.sleep(3000);

		driver.findElement(savbunitrresult).click();
		Thread.sleep(3000);

		driver.findElement(SideBarr).click();
		Thread.sleep(3000);

		driver.findElement(Microbilogy).click();
		Thread.sleep(3000);

		driver.findElement(approve).click();
		Thread.sleep(3000);

		driver.findElement(Checkboixfdd).click();
		Thread.sleep(3000);

		driver.findElement(BulkEntry).click();
		Thread.sleep(3000);

		Thread.sleep(1000);

	//	String kke = driver.findElement(Recollectegettext).getText();
/*
		String Actual = kke;
		String Expected = kke;

		Assert.assertEquals(Actual, Expected, "Verify Normal Result");
*/

	}
	

	public void ResultEntryDocumentUploadVerifynewupload() throws InterruptedException {




		WebDriverWait wait = new WebDriverWait(driver,6000);
		/*

		Thread.sleep(8000); 

		driver.findElement(mainmenu2).click(); 
		Thread.sleep(8000);
		driver.findElement(OPD3).click(); 
		Thread.sleep(8000);

		//Thread.sleep(1000);
		driver.findElement(SideBarMenu).click();
		Thread.sleep(2000);
		driver.findElement(RegistrationMenu).click();
		Thread.sleep(2000); 
		driver.findElement(EncounterListw).click();
		Thread.sleep(3000);

		String takename = driver.findElement(getpataientname).getText();

		 */
		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();

		Thread.sleep(3000);
		WebElement Expand1 = wait.until(ExpectedConditions.elementToBeClickable(Expand)); 
		Expand1.click();
		//	Thread.sleep(3000);
		WebElement mrnnn1 =wait.until(ExpectedConditions.elementToBeClickable(name));
		mrnnn1.sendKeys("PATHOLOGY"); 
		//Thread.sleep(3000);
		WebElement sercahghh1 =wait.until(ExpectedConditions.elementToBeClickable(sercahghh));
		sercahghh1.click();
		//Thread.sleep(3000);
		WebElement selectreco1 = wait.until(ExpectedConditions.elementToBeClickable(selectreco));
		selectreco1.click();
		WebElement selectrecheckbox1 = wait.until(ExpectedConditions.elementToBeClickable(selectrecheckbox));
		selectrecheckbox1.click();
		WebElement Printandsample1 = wait.until(ExpectedConditions.elementToBeClickable(Printandsample));
		Printandsample1.click();
		WebElement saveandsubmit1 = wait.until(ExpectedConditions.elementToBeClickable(saveandsubmit));
		saveandsubmit1.click();
		//Thread.sleep(2000);
		String windowHandle6 = driver.getWindowHandle();
		ArrayList<Object> tabs2u = new ArrayList<Object> (driver.getWindowHandles());
		driver.switchTo().window((String) tabs2u.get(0));
		//Thread.sleep(2000);

		WebElement SideBarr1 = wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr1.click();
		//	Thread.sleep(7000);
		WebElement centerreceving1 = wait.until(ExpectedConditions.elementToBeClickable(centerreceving));
		centerreceving1.click();
		driver.navigate().refresh();
		WebElement centerrecivecwhck1 = wait.until(ExpectedConditions.elementToBeClickable(centerrecivecwhck));
		centerrecivecwhck1.click();
		WebElement receiversubmit1 = wait.until(ExpectedConditions.elementToBeClickable(receiversubmit));
		receiversubmit1.click();
		WebElement receivesample1 = wait.until(ExpectedConditions.elementToBeClickable(receivesample));
		receivesample1.click();
		Thread.sleep(3000);

		WebElement SideBarr122= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr122.click();		
		//Thread.sleep(2000);

		WebElement ResultEntry11 = wait.until(ExpectedConditions.elementToBeClickable(ResultEntry1));
		ResultEntry11.click();

		WebElement checkboocx1 = wait.until(ExpectedConditions.elementToBeClickable(checkboocx));
		checkboocx1.click();

		WebElement Result_entryupbuton1 = wait.until(ExpectedConditions.elementToBeClickable(Result_entryupbuton));
		Result_entryupbuton1.click();

		WebElement InternalDemo1 = wait.until(ExpectedConditions.elementToBeClickable(InternalDemo));
		InternalDemo1.sendKeys("Enter Abnormal Result");

		WebElement selectrecoo1 = wait.until(ExpectedConditions.elementToBeClickable(selectrecoo));
		selectrecoo1.click();

		WebElement Parameterinput11 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput1));
		Parameterinput11.sendKeys("4");
		Thread.sleep(1000);

		WebElement Parameterinput22 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput2));
		Parameterinput22.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput31 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput3));
		Parameterinput31.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput41 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput4));
		Parameterinput41.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput52 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput5));
		Parameterinput52.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput66 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput6));
		Parameterinput66.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput71 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput7));
		Parameterinput71.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput81 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput8));
		Parameterinput81.sendKeys("4");
		Thread.sleep(1000);	
		WebElement Parameterinput91 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput9));
		Parameterinput91.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput101 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput10));
		Parameterinput101.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput1141 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput111));
		Parameterinput1141.sendKeys("4");

		WebElement Clickoncheckbox1 = wait.until(ExpectedConditions.elementToBeClickable(Clickoncheckbox));
		Clickoncheckbox1.click();
		//Thread.sleep(1000);
		WebElement UploadResultEntrybutton11 = wait.until(ExpectedConditions.elementToBeClickable(UploadResultEntrybutton1));
		UploadResultEntrybutton11.click();
		//Thread.sleep(1000);
		WebElement UploadResultEntrybutton21 = wait.until(ExpectedConditions.elementToBeClickable(UploadResultEntrybutton2));
		UploadResultEntrybutton21.click();
		//Thread.sleep(1000);
		WebElement CheeoseFile_41 = wait.until(ExpectedConditions.elementToBeClickable(CheeoseFile_4));
		CheeoseFile_41.click();
		//Thread.sleep(1000);
		WebElement Upload31 = wait.until(ExpectedConditions.elementToBeClickable(Upload3));
		Upload31.click();
		//Thread.sleep(1000);
		WebElement SaveButton51 = wait.until(ExpectedConditions.elementToBeClickable(SaveButton5));
		SaveButton51.click();

		WebElement ClickOnsAVE1 = wait.until(ExpectedConditions.elementToBeClickable(ClickOnsAVE));
		ClickOnsAVE1.click();

		//Thread.sleep(2000);
		WebElement back1 = wait.until(ExpectedConditions.elementToBeClickable(back));
		back1.click();

		WebElement SideBarr123= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr123.click();	

		//Thread.sleep(1000);

		//WebElement Pathology11 = wait.until(ExpectedConditions.elementToBeClickable(Pathology));
		//Pathology11.click();
		//Thread.sleep(1000);

		WebElement ResultEntry111 = wait.until(ExpectedConditions.elementToBeClickable(ResultEntry1));
		ResultEntry111.click();


		WebElement checkboocx11 = wait.until(ExpectedConditions.elementToBeClickable(checkboocx));
		checkboocx11.click();


		WebElement Result_entryupbuton11 = wait.until(ExpectedConditions.elementToBeClickable(Result_entryupbuton));
		Result_entryupbuton11.click();


		//WebElement SideBarr1233= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		//SideBarr1233.click();
		//Thread.sleep(2000);
		WebElement checkboocx1343 = wait.until(ExpectedConditions.elementToBeClickable(Checkboxt));
		checkboocx1343.click();

		WebElement submitbut1 = wait.until(ExpectedConditions.elementToBeClickable(submitbut));
		submitbut1.click();

		WebElement SideBarr1242= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr1242.click();
		Thread.sleep(1000);

		WebElement ResultValidation1 = wait.until(ExpectedConditions.elementToBeClickable(ResultValidation));
		ResultValidation1.click();

		//Thread.sleep(3000);

		WebElement ClickOncheckbox2 = wait.until(ExpectedConditions.elementToBeClickable(ClickOncheckbox));
		ClickOncheckbox2.click();

		//Thread.sleep(5000);
		driver.findElement(ThreeDot).click();
		Thread.sleep(5000);
		driver.findElement(UploadDocumet).click();
		Thread.sleep(5000);

		String kk = driver.findElement(FileNameAssertion).getText();
		String Actaul = kk;
		String Expected = kk;
		Assert.assertEquals(Actaul, Expected, "File verified successfully");
		Thread.sleep(5000);
		driver.findElement(Uploaddocumet_2).click();
		Thread.sleep(8000);
		WebElement browse = driver.findElement(ChooseFileValidation);
		//click on ‘Choose file’ to upload the desired file

		String projectpath = System.getProperty("user.dir");
		StringSelection ss = new StringSelection(projectpath + "\\Text file\\Demo.txt");
		browse.sendKeys(projectpath + "\\Text file\\Demo.txt");	
		Thread.sleep(5000);
		driver.findElement(Uploadnewfile).click();
		Thread.sleep(5000);

		String IssueListindvrify = driver.findElement(NewFileValidation).getText();
		String Actuall = IssueListindvrify;
		String Expectedd = IssueListindvrify;
		Assert.assertEquals(Actuall, Expectedd, "Documennt uploaded sucessfully");
		Thread.sleep(3000);
		driver.findElement(SaveValidation).click();
		Thread.sleep(5000);

	}

public void Retest_in_result_entry() throws InterruptedException {



		WebDriverWait wait = new WebDriverWait(driver,8000);



		/*
				Thread.sleep(1000); 

				driver.findElement(mainmenu2).click(); 
				Thread.sleep(8000);
				driver.findElement(OPD3).click(); 
				Thread.sleep(8000);

				//Thread.sleep(1000);
				driver.findElement(SideBarMenu).click();
				Thread.sleep(2000);
				driver.findElement(RegistrationMenu).click();
				Thread.sleep(2000); 
				driver.findElement(EncounterListw).click();
				Thread.sleep(3000);

				String takename = driver.findElement(getpataientname).getText();
		 */

		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();

		Thread.sleep(3000);
		WebElement Expand1 = wait.until(ExpectedConditions.elementToBeClickable(Expand)); 
		Expand1.click();

		//driver.findElement(Expand).click();
		//Thread.sleep(3000);
		WebElement mrnnn1 =wait.until(ExpectedConditions.elementToBeClickable(name));
		mrnnn1.sendKeys("PATHOLOGY"); 
		//driver.findElement(name).sendKeys("PATHOLOGY"); 
		//Thread.sleep(2000);
		//Thread.sleep(3000);
		WebElement sercahghh1 =wait.until(ExpectedConditions.elementToBeClickable(sercahghh));
		sercahghh1.click();
		//driver.findElement(sercahghh).click();
		//Thread.sleep(2000);
		WebElement selectreco1 = wait.until(ExpectedConditions.elementToBeClickable(selectreco));
		selectreco1.click();
		//driver.findElement(selectreco).click();
		//Thread.sleep(2000);
		WebElement selectrecheckbox1 = wait.until(ExpectedConditions.elementToBeClickable(selectrecheckbox));
		selectrecheckbox1.click();
		//driver.findElement(selectrecheckbox).click();
		//Thread.sleep(2000);
		WebElement Printandsample1 = wait.until(ExpectedConditions.elementToBeClickable(Printandsample));
		Printandsample1.click();
		//driver.findElement(Printandsample).click();
		//Thread.sleep(2000);
		WebElement saveandsubmit1 = wait.until(ExpectedConditions.elementToBeClickable(saveandsubmit));
		saveandsubmit1.click();
		//driver.findElement(saveandsubmit).click();
		//Thread.sleep(2000);
		String windowHandle6 = driver.getWindowHandle();
		ArrayList<Object> tabs2u = new ArrayList<Object> (driver.getWindowHandles());
		driver.switchTo().window((String) tabs2u.get(0));
		//Thread.sleep(2000);

		WebElement SideBarr1 = wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr1.click();
		//driver.findElement(SideBarr).click();
		//Thread.sleep(2000);
		//driver.navigate().refresh();
		WebElement centerreceving1 = wait.until(ExpectedConditions.elementToBeClickable(centerreceving));
		centerreceving1.click();
		driver.navigate().refresh();
		//driver.findElement(centerreceving).click();
		//Thread.sleep(2000);
		WebElement centerrecivecwhck1 = wait.until(ExpectedConditions.elementToBeClickable(centerrecivecwhck));
		centerrecivecwhck1.click();
		//Thread.sleep(2000);
		//driver.findElement(centerrecivecwhck).click();
		WebElement receiversubmit1 = wait.until(ExpectedConditions.elementToBeClickable(receiversubmit));
		receiversubmit1.click();
		//driver.findElement(receiversubmit).click();
		//Thread.sleep(2000);
		WebElement receivesample1 = wait.until(ExpectedConditions.elementToBeClickable(receivesample));
		receivesample1.click();
		//Thread.sleep(2000);
		//	driver.findElement(receivesample).click();


		Thread.sleep(2000);

		WebElement SideBarr12= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr12.click();
		Thread.sleep(2000);

		WebElement Pathology1 = wait.until(ExpectedConditions.elementToBeClickable(Pathology));
		Pathology1.click();
		WebElement sampleprocessing1 = wait.until(ExpectedConditions.elementToBeClickable(sampleprocessing));
		sampleprocessing1.click();
		Thread.sleep(3000);
		WebElement checkclcik1 = wait.until(ExpectedConditions.elementToBeClickable(checkclcik));
		checkclcik1.click();
		Thread.sleep(3000);
		WebElement Acceptsample1 = wait.until(ExpectedConditions.elementToBeClickable(Acceptsample));
		Acceptsample1.click();
		Thread.sleep(3000);
		WebElement Acceptsamplesave1 = wait.until(ExpectedConditions.elementToBeClickable(Acceptsamplesave));
		Acceptsamplesave1.click();

		Thread.sleep(3000);

		WebElement SideBarr122= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr122.click();		
		//Thread.sleep(2000);

		WebElement ResultEntry11 = wait.until(ExpectedConditions.elementToBeClickable(ResultEntry1));
		ResultEntry11.click();

		//Thread.sleep(2000);

		//String jj = driver.findElement(getpatiaentnameu).getText();

		WebElement checkboocx1 = wait.until(ExpectedConditions.elementToBeClickable(checkboocx));
		checkboocx1.click();

		WebElement Result_entryupbuton1 = wait.until(ExpectedConditions.elementToBeClickable(Result_entryupbuton));
		Result_entryupbuton1.click();

		WebElement InternalDemo1 = wait.until(ExpectedConditions.elementToBeClickable(InternalDemo));
		InternalDemo1.sendKeys("Enter Normal Result");

		WebElement selectrecoo1 = wait.until(ExpectedConditions.elementToBeClickable(selectrecoo));
		selectrecoo1.click();

		WebElement Parameterinput11 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput1));
		Parameterinput11.sendKeys("4");
		Thread.sleep(1000);

		WebElement Parameterinput22 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput2));
		Parameterinput22.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput31 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput3));
		Parameterinput31.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput41 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput4));
		Parameterinput41.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput52 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput5));
		Parameterinput52.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput66 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput6));
		Parameterinput66.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput71 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput7));
		Parameterinput71.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput81 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput8));
		Parameterinput81.sendKeys("4");
		Thread.sleep(1000);	
		WebElement Parameterinput91 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput9));
		Parameterinput91.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput101 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput10));
		Parameterinput101.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput1141 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput111));
		Parameterinput1141.sendKeys("4");
		Thread.sleep(1000);		

		WebElement Clickoncheckbox1 = wait.until(ExpectedConditions.elementToBeClickable(Clickoncheckbox));
		Clickoncheckbox1.click();
		Thread.sleep(2000);	
		WebElement ClickOnsAVE1 = wait.until(ExpectedConditions.elementToBeClickable(ClickOnsAVE));
		ClickOnsAVE1.click();
		Thread.sleep(2000);	
		WebElement Clickoncheckbox2 = wait.until(ExpectedConditions.elementToBeClickable(Clickoncheckbox));
		Clickoncheckbox2.click();
		Thread.sleep(2000);	
		WebElement Reset1 = wait.until(ExpectedConditions.elementToBeClickable(Reset));
		Reset1.click();
		Thread.sleep(2000);	
		/*
		WebElement RBCCheckbox1 = wait.until(ExpectedConditions.elementToBeClickable(RBCCheckbox));
		RBCCheckbox1.click();
		Thread.sleep(2000);	
		WebElement comments1 = wait.until(ExpectedConditions.elementToBeClickable(comments));
		comments1.sendKeys("retest");
		Thread.sleep(2000);	
		WebElement ClickOnReason1 = wait.until(ExpectedConditions.elementToBeClickable(ClickOnReason));
		ClickOnReason1.click();

		WebElement SampleRestest1 = wait.until(ExpectedConditions.elementToBeClickable(SampleRestest));
		SampleRestest1.click();

		WebElement Confirmmm1 = wait.until(ExpectedConditions.elementToBeClickable(Confirmmm));
		Confirmmm1.click();

		WebElement checkbow1 = wait.until(ExpectedConditions.elementToBeClickable(checkbow));
		checkbow1.click();


		//WebElement ResultEntry111 = wait.until(ExpectedConditions.elementToBeClickable(ResultEntry1));
		//ResultEntry111.click();


		//WebElement checkboocx11 = wait.until(ExpectedConditions.elementToBeClickable(checkboocx));
		//checkboocx11.click();


		WebElement Result_entryupbuton11 = wait.until(ExpectedConditions.elementToBeClickable(Result_entryupbuton));
		Result_entryupbuton11.click();


		//WebElement SideBarr1233= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		//SideBarr1233.click();
		//Thread.sleep(2000);
		WebElement checkboocx1343 = wait.until(ExpectedConditions.elementToBeClickable(Checkboxt));
		checkboocx1343.click();

		WebElement parameterforretest1 = wait.until(ExpectedConditions.elementToBeClickable(parameterforretest));
		parameterforretest1.sendKeys("5");
		//Thread.sleep(1000);

		WebElement submitbut1 = wait.until(ExpectedConditions.elementToBeClickable(submitbut));
		submitbut1.click();

		WebElement SideBarr1242= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr1242.click();
		//Thread.sleep(1000);

		WebElement ResultValidation1 = wait.until(ExpectedConditions.elementToBeClickable(ResultValidation));
		ResultValidation1.click();

		//Thread.sleep(3000);

		WebElement ClickOncheckbox2 = wait.until(ExpectedConditions.elementToBeClickable(ClickOncheckbox));
		ClickOncheckbox2.click();

		WebElement Result_entryupbuton121 = wait.until(ExpectedConditions.elementToBeClickable(Result_entryupbuton));
		Result_entryupbuton121.click();
		//Thread.sleep(1000);

		WebElement Clickon_checkbox2 = wait.until(ExpectedConditions.elementToBeClickable(Clickon_checkbox));
		Clickon_checkbox2.click();

		WebElement Clickonsubmit3 = wait.until(ExpectedConditions.elementToBeClickable(Clickonsubmit));
		Clickonsubmit3.click();

		WebElement SideBarr124k2= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr124k2.click();
		//Thread.sleep(1000);

		WebElement ResultApporoavl2= wait.until(ExpectedConditions.elementToBeClickable(ResultApporoavl));
		ResultApporoavl2.click();

		//Thread.sleep(3000);

		//String kk = driver.findElement(getpatiaentname).getText();


		WebElement ResultApprovalCheckbox1= wait.until(ExpectedConditions.elementToBeClickable(ResultApprovalCheckbox));
		ResultApprovalCheckbox1.click();

		WebElement Result_entryupbuton19 = wait.until(ExpectedConditions.elementToBeClickable(Result_entryupbuton));
		Result_entryupbuton19.click();

		WebElement Resultapprovalcheckboxsubmit9 = wait.until(ExpectedConditions.elementToBeClickable(Resultapprovalcheckboxsubmit));
		Resultapprovalcheckboxsubmit9.click();

		WebElement Submmit2 = wait.until(ExpectedConditions.elementToBeClickable(Submmit));
		Submmit2.click();

		WebElement SideBarr124k765= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr124k765.click();

		WebElement Report1= wait.until(ExpectedConditions.elementToBeClickable(Report));
		Report1.click();

		//Thread.sleep(3000);
		WebElement Expand3 = wait.until(ExpectedConditions.elementToBeClickable(Expand)); 
		Expand3.click();
		//Thread.sleep(3000);
		WebElement mrnnn3 =wait.until(ExpectedConditions.elementToBeClickable(name));
		mrnnn3.sendKeys("PATHOLOGY"); 
		//Thread.sleep(3000);
		WebElement sercahghh3 =wait.until(ExpectedConditions.elementToBeClickable(sercahghh));
		sercahghh3.click();
		Thread.sleep(4000);


		String ll = driver.findElement(Patientnameget).getText();

		String Actual = ll;
		String Expected = ll;

		Assert.assertEquals(Actual, Expected, "Retested pataient has been verified successfully");
		 */
	}



public void Upload_Document_to_Approval() throws InterruptedException {



		WebDriverWait wait = new WebDriverWait(driver,6000);
		/*

		Thread.sleep(8000); 

		driver.findElement(mainmenu2).click(); 
		Thread.sleep(8000);
		driver.findElement(OPD3).click(); 
		Thread.sleep(8000);

		//Thread.sleep(1000);
		driver.findElement(SideBarMenu).click();
		Thread.sleep(2000);
		driver.findElement(RegistrationMenu).click();
		Thread.sleep(2000); 
		driver.findElement(EncounterListw).click();
		Thread.sleep(3000);

		String takename = driver.findElement(getpataientname).getText();

		 */
		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();

		Thread.sleep(3000);
		WebElement Expand1 = wait.until(ExpectedConditions.elementToBeClickable(Expand)); 
		Expand1.click();
		//	Thread.sleep(3000);
		WebElement mrnnn1 =wait.until(ExpectedConditions.elementToBeClickable(name));
		mrnnn1.sendKeys("PATHOLOGY"); 
		//Thread.sleep(3000);
		WebElement sercahghh1 =wait.until(ExpectedConditions.elementToBeClickable(sercahghh));
		sercahghh1.click();
		//Thread.sleep(3000);
		WebElement selectreco1 = wait.until(ExpectedConditions.elementToBeClickable(selectreco));
		selectreco1.click();
		WebElement selectrecheckbox1 = wait.until(ExpectedConditions.elementToBeClickable(selectrecheckbox));
		selectrecheckbox1.click();
		WebElement Printandsample1 = wait.until(ExpectedConditions.elementToBeClickable(Printandsample));
		Printandsample1.click();
		WebElement saveandsubmit1 = wait.until(ExpectedConditions.elementToBeClickable(saveandsubmit));
		saveandsubmit1.click();
		//Thread.sleep(2000);
		String windowHandle6 = driver.getWindowHandle();
		ArrayList<Object> tabs2u = new ArrayList<Object> (driver.getWindowHandles());
		driver.switchTo().window((String) tabs2u.get(0));
		//Thread.sleep(2000);

		WebElement SideBarr1 = wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr1.click();
		//	Thread.sleep(7000);
		WebElement centerreceving1 = wait.until(ExpectedConditions.elementToBeClickable(centerreceving));
		centerreceving1.click();
		driver.navigate().refresh();
		WebElement centerrecivecwhck1 = wait.until(ExpectedConditions.elementToBeClickable(centerrecivecwhck));
		centerrecivecwhck1.click();
		WebElement receiversubmit1 = wait.until(ExpectedConditions.elementToBeClickable(receiversubmit));
		receiversubmit1.click();
		WebElement receivesample1 = wait.until(ExpectedConditions.elementToBeClickable(receivesample));
		receivesample1.click();

		Thread.sleep(3000);

		WebElement SideBarr12= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr12.click();
		Thread.sleep(7000);

		WebElement Pathology1 = wait.until(ExpectedConditions.elementToBeClickable(Pathology));
		Pathology1.click();
		WebElement sampleprocessing1 = wait.until(ExpectedConditions.elementToBeClickable(sampleprocessing));
		sampleprocessing1.click();
		//Thread.sleep(3000);
		WebElement checkclcik1 = wait.until(ExpectedConditions.elementToBeClickable(checkclcik));
		checkclcik1.click();
		WebElement Acceptsample1 = wait.until(ExpectedConditions.elementToBeClickable(Acceptsample));
		Acceptsample1.click();
		WebElement Acceptsamplesave1 = wait.until(ExpectedConditions.elementToBeClickable(Acceptsamplesave));
		Acceptsamplesave1.click();

		//Thread.sleep(3000);

		WebElement SideBarr122= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr122.click();		
		//Thread.sleep(2000);

		WebElement ResultEntry11 = wait.until(ExpectedConditions.elementToBeClickable(ResultEntry1));
		ResultEntry11.click();

		WebElement checkboocx1 = wait.until(ExpectedConditions.elementToBeClickable(Checkboxchab));
		checkboocx1.click();

		WebElement Result_entryupbuton1 = wait.until(ExpectedConditions.elementToBeClickable(Result_entryupbuton));
		Result_entryupbuton1.click();

		WebElement InternalDemo1 = wait.until(ExpectedConditions.elementToBeClickable(InternalDemo));
		InternalDemo1.sendKeys("Enter Normal Result");

		WebElement selectrecoo1 = wait.until(ExpectedConditions.elementToBeClickable(selectrecoo));
		selectrecoo1.click();

		WebElement Parameterinput11 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput1));
		Parameterinput11.sendKeys("4");
		Thread.sleep(1000);

		WebElement Parameterinput22 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput2));
		Parameterinput22.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput31 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput3));
		Parameterinput31.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput41 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput4));
		Parameterinput41.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput52 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput5));
		Parameterinput52.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput66 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput6));
		Parameterinput66.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput71 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput7));
		Parameterinput71.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput81 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput8));
		Parameterinput81.sendKeys("4");
		Thread.sleep(1000);	
		WebElement Parameterinput91 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput9));
		Parameterinput91.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput101 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput10));
		Parameterinput101.sendKeys("4");
		Thread.sleep(1000);
		WebElement Parameterinput1141 = wait.until(ExpectedConditions.elementToBeClickable(Parameterinput111));
		Parameterinput1141.sendKeys("4");
		Thread.sleep(2000);

		WebElement Clickoncheckbox1 = wait.until(ExpectedConditions.elementToBeClickable(Clickoncheckbox));
		Clickoncheckbox1.click();
		Thread.sleep(2000);
		WebElement UploadResultEntrybutton11 = wait.until(ExpectedConditions.elementToBeClickable(UploadResultEntrybutton1));
		UploadResultEntrybutton11.click();
		Thread.sleep(1000);
		WebElement UploadResultEntrybutton21 = wait.until(ExpectedConditions.elementToBeClickable(UploadResultEntrybutton2));
		UploadResultEntrybutton21.click();
		//Thread.sleep(3000);

		Thread.sleep(8000);
		WebElement browse = driver.findElement(CheeoseFile_4);
		//click on ‘Choose file’ to upload the desired file

		String projectpath = System.getProperty("user.dir");
		StringSelection ss = new StringSelection(projectpath + "\\Text file\\Demo.txt");
		browse.sendKeys(projectpath + "\\Text file\\Demo.txt");	


		//WebElement CheeoseFile_41 = wait.until(ExpectedConditions.elementToBeClickable(CheeoseFile_4));
		//CheeoseFile_41.click();
		Thread.sleep(1000);
		WebElement Upload31 = wait.until(ExpectedConditions.elementToBeClickable(Upload3));
		Upload31.click();
		Thread.sleep(1000);
		WebElement SaveButton51 = wait.until(ExpectedConditions.elementToBeClickable(SaveButton5));
		SaveButton51.click();
		Thread.sleep(2000);
		WebElement ClickOnsAVE1 = wait.until(ExpectedConditions.elementToBeClickable(ClickOnsAVE));
		ClickOnsAVE1.click();

		Thread.sleep(2000);
		WebElement back1 = wait.until(ExpectedConditions.elementToBeClickable(back));
		back1.click();

		WebElement SideBarr123= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr123.click();	

		Thread.sleep(1000);

		//WebElement Pathology11 = wait.until(ExpectedConditions.elementToBeClickable(Pathology));
		//Pathology11.click();
		//Thread.sleep(1000);

		WebElement ResultEntry111 = wait.until(ExpectedConditions.elementToBeClickable(ResultEntry1));
		ResultEntry111.click();

		Thread.sleep(2000);
		WebElement checkboocx11 = wait.until(ExpectedConditions.elementToBeClickable(checkboocx));
		checkboocx11.click();

		Thread.sleep(2000);
		WebElement Result_entryupbuton11 = wait.until(ExpectedConditions.elementToBeClickable(Result_entryupbuton));
		Result_entryupbuton11.click();
		Thread.sleep(2000);

		//WebElement SideBarr1233= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		//SideBarr1233.click();
		//Thread.sleep(2000);
		WebElement checkboocx1343 = wait.until(ExpectedConditions.elementToBeClickable(Checkboxt));
		checkboocx1343.click();
		Thread.sleep(2000);
		WebElement submitbut1 = wait.until(ExpectedConditions.elementToBeClickable(submitbut));
		submitbut1.click();
		Thread.sleep(2000);
		WebElement SideBarr1242= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr1242.click();
		//Thread.sleep(1000);
		Thread.sleep(2000);
		WebElement ResultValidation1 = wait.until(ExpectedConditions.elementToBeClickable(ResultValidation));
		ResultValidation1.click();
		Thread.sleep(2000);
		//Thread.sleep(3000);

		WebElement ClickOncheckbox2 = wait.until(ExpectedConditions.elementToBeClickable(ClickOncheckbox));
		ClickOncheckbox2.click();
		Thread.sleep(2000);
		//Thread.sleep(5000);
		driver.findElement(ThreeDot).click();
		Thread.sleep(5000);
		driver.findElement(UploadDocumet).click();
		Thread.sleep(5000);

		String kk = driver.findElement(FileNameAssertion).getText();
		String Actaul = kk;
		String Expected = kk;
		Assert.assertEquals(Actaul, Expected, "File verified successfully");

		/*
		Thread.sleep(5000);
		driver.findElement(Uploaddocumet_2).click();
		Thread.sleep(8000);
		WebElement browse3 = driver.findElement(ChooseFileValidation);
		//click on ‘Choose file’ to upload the desired file

		String projectpath1 = System.getProperty("user.dir");
		StringSelection ss1 = new StringSelection(projectpath1 + "\\Text file\\Demo.txt");
		browse3.sendKeys(projectpath1 + "\\Text file\\Demo.txt");	
		Thread.sleep(5000);
		driver.findElement(Uploadnewfile).click();
		Thread.sleep(5000);

		String IssueListindvrify = driver.findElement(NewFileValidation).getText();
		String Actuall = IssueListindvrify;
		String Expectedd = IssueListindvrify;
		Assert.assertEquals(Actuall, Expectedd, "Documennt uploaded sucessfully");
		Thread.sleep(3000);
		driver.findElement(SaveValidation).click();
		//Thread.sleep(5000);
		WebElement SideBarr1pp242= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr1pp242.click();
		Thread.sleep(1000);
		WebElement ResulApproval1= wait.until(ExpectedConditions.elementToBeClickable(ResulApproval));
		ResulApproval1.click();
		Thread.sleep(2000);
		WebElement ResultApporvalCheckbox1= wait.until(ExpectedConditions.elementToBeClickable(ResultApporvalCheckbox));
		ResultApporvalCheckbox1.click();
		Thread.sleep(2000);
		WebElement Result_entryupbuton1i1 = wait.until(ExpectedConditions.elementToBeClickable(Result_entryupbuton));
		Result_entryupbuton1i1.click();
		Thread.sleep(2000);
		WebElement SecondCheckboxClikc1= wait.until(ExpectedConditions.elementToBeClickable(SecondCheckboxClikc));
		SecondCheckboxClikc1.click();
		Thread.sleep(2000);
		WebElement Clickonrecord1= wait.until(ExpectedConditions.elementToBeClickable(Clickonrecord));
		Clickonrecord1.click();
		Thread.sleep(2000);
		WebElement Uploaddoc1= wait.until(ExpectedConditions.elementToBeClickable(Uploaddoc));
		Uploaddoc1.click();
		Thread.sleep(2000);
		WebElement Uploaddocumet1= wait.until(ExpectedConditions.elementToBeClickable(Uploaddocumet));
		Uploaddocumet1.click();
		Thread.sleep(2000);
		WebElement browse1 = driver.findElement(CheeoseFille);
		//click on ‘Choose file’ to upload the desired file
		String projectpathj = System.getProperty("user.dir");
		StringSelection sss = new StringSelection(projectpathj + "\\Text file\\Demo.txt");
		browse1.sendKeys(projectpathj + "\\Text file\\Demo.txt");	
		Thread.sleep(5000);
		driver.findElement(Uploadd).click();

		Thread.sleep(5000);

		//String bb = driver.findElement(GetapprovalFilename);

		String IssueListindvrify1 = driver.findElement(GetapprovalFilename).getText();
		String Actualll = IssueListindvrify1;
		String Expecteddd = IssueListindvrify1;
		Assert.assertEquals(Actualll, Expecteddd, "Documennt uploaded sucessfully");
		Thread.sleep(2000);
		WebElement Savee1= wait.until(ExpectedConditions.elementToBeClickable(Savee));
		Savee1.click();


		 */


public void rejetOrderVedetails () throws InterruptedException {



		WebDriverWait wait = new WebDriverWait(driver,8000);



		/*
				Thread.sleep(1000); 

				driver.findElement(mainmenu2).click(); 
				Thread.sleep(8000);
				driver.findElement(OPD3).click(); 
				Thread.sleep(8000);

				//Thread.sleep(1000);
				driver.findElement(SideBarMenu).click();
				Thread.sleep(2000);
				driver.findElement(RegistrationMenu).click();
				Thread.sleep(2000); 
				driver.findElement(EncounterListw).click();
				Thread.sleep(3000);

				String takename = driver.findElement(getpataientname).getText();
		 */

		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();

		Thread.sleep(3000);
		WebElement Expand1 = wait.until(ExpectedConditions.elementToBeClickable(Expand)); 
		Expand1.click();

		//driver.findElement(Expand).click();
		//Thread.sleep(3000);
		WebElement mrnnn1 =wait.until(ExpectedConditions.elementToBeClickable(name));
		mrnnn1.sendKeys("PATHOLOGY"); 
		//driver.findElement(name).sendKeys("PATHOLOGY"); 
		//Thread.sleep(2000);
		//Thread.sleep(3000);
		WebElement sercahghh1 =wait.until(ExpectedConditions.elementToBeClickable(sercahghh));
		sercahghh1.click();
		//driver.findElement(sercahghh).click();
		//Thread.sleep(2000);
		WebElement selectreco1 = wait.until(ExpectedConditions.elementToBeClickable(selectreco));
		selectreco1.click();
		//driver.findElement(selectreco).click();
		//Thread.sleep(2000);
		WebElement selectrecheckbox1 = wait.until(ExpectedConditions.elementToBeClickable(selectrecheckbox));
		selectrecheckbox1.click();
		//driver.findElement(selectrecheckbox).click();
		//Thread.sleep(2000);
		WebElement Printandsample1 = wait.until(ExpectedConditions.elementToBeClickable(Printandsample));
		Printandsample1.click();
		//driver.findElement(Printandsample).click();
		//Thread.sleep(2000);
		WebElement saveandsubmit1 = wait.until(ExpectedConditions.elementToBeClickable(saveandsubmit));
		saveandsubmit1.click();
		//driver.findElement(saveandsubmit).click();
		//Thread.sleep(2000);
		String windowHandle6 = driver.getWindowHandle();
		ArrayList<Object> tabs2u = new ArrayList<Object> (driver.getWindowHandles());
		driver.switchTo().window((String) tabs2u.get(0));
		//Thread.sleep(2000);

		WebElement SideBarr1 = wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr1.click();
		//driver.findElement(SideBarr).click();
		//Thread.sleep(2000);
		//driver.navigate().refresh();
		WebElement centerreceving1 = wait.until(ExpectedConditions.elementToBeClickable(centerreceving));
		centerreceving1.click();
		driver.navigate().refresh();
		//driver.findElement(centerreceving).click();
		//Thread.sleep(2000);
		WebElement centerrecivecwhck1 = wait.until(ExpectedConditions.elementToBeClickable(centerrecivecwhck));
		centerrecivecwhck1.click();
		//Thread.sleep(2000);
		//driver.findElement(centerrecivecwhck).click();
		WebElement receiversubmit1 = wait.until(ExpectedConditions.elementToBeClickable(receiversubmit));
		receiversubmit1.click();
		//driver.findElement(receiversubmit).click();
		//Thread.sleep(2000);
		WebElement receivesample1 = wait.until(ExpectedConditions.elementToBeClickable(receivesample));
		receivesample1.click();
		//Thread.sleep(2000);
		//	driver.findElement(receivesample).click();


		Thread.sleep(2000);

		WebElement SideBarr12= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr12.click();
		Thread.sleep(2000);

		WebElement Pathology1 = wait.until(ExpectedConditions.elementToBeClickable(Pathology));
		Pathology1.click();
		WebElement sampleprocessing1 = wait.until(ExpectedConditions.elementToBeClickable(sampleprocessing));
		sampleprocessing1.click();
		Thread.sleep(3000);
		WebElement checkclcik1 = wait.until(ExpectedConditions.elementToBeClickable(checkclcik));
		checkclcik1.click();
		Thread.sleep(3000);
		WebElement Acceptsample1 = wait.until(ExpectedConditions.elementToBeClickable(Acceptsample));
		Acceptsample1.click();
		Thread.sleep(3000);
		WebElement Acceptsamplesave1 = wait.until(ExpectedConditions.elementToBeClickable(Acceptsamplesave));
		Acceptsamplesave1.click();

		Thread.sleep(3000);

		WebElement SideBarr122= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr122.click();		
		//Thread.sleep(2000);
		Thread.sleep(2000);
		WebElement ResultEntry11 = wait.until(ExpectedConditions.elementToBeClickable(ResultEntry1));
		ResultEntry11.click();

		//Thread.sleep(2000);
		// ResultEntry1= wait.until(ExpectedConditions.elementToBeClickable(ResultEntry));
		//ResultEntry1.click();
		//Thread.sleep(1000);

		Thread.sleep(2000);
		WebElement checkboxdd1= wait.until(ExpectedConditions.elementToBeClickable(checkboxdd));
		checkboxdd1.click();

		//Thread.sleep(3000);

		//String getholddatanmae1 = driver.findElement(getholddatanmae).getText();

		WebElement Reject1= wait.until(ExpectedConditions.elementToBeClickable(Reject));
		Reject1.click();
		//Thread.sleep(4000);
		Thread.sleep(2000);
		WebElement Selectreason1= wait.until(ExpectedConditions.elementToBeClickable(Selectreason));
		Selectreason1.click();
		Thread.sleep(2000);

		WebElement Resaonoption1= wait.until(ExpectedConditions.elementToBeClickable(Resaonoption));
		Resaonoption1.click();
		Thread.sleep(2000);

		WebElement Rejectbutton1= wait.until(ExpectedConditions.elementToBeClickable(Rejectbutton));
		Rejectbutton1.click();
		//Thread.sleep(5000);
		Thread.sleep(2000);

		WebElement SideBarrwf1r = wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarrwf1r.click();
		Thread.sleep(2000);
		//Thread.sleep(1000);

		WebElement SampleTrackingg1 = wait.until(ExpectedConditions.elementToBeClickable(SampleTrackingg));
		SampleTrackingg1.click();
		//	Thread.sleep(1000);
		Thread.sleep(2000);
		WebElement Expand11 = wait.until(ExpectedConditions.elementToBeClickable(Expand)); 
		Expand11.click();
		Thread.sleep(2000);
		//Thread.sleep(3000);
		WebElement mrnnn11 =wait.until(ExpectedConditions.elementToBeClickable(name));
		mrnnn11.sendKeys("PATHOLOGY"); 
		//Thread.sleep(3000);
		WebElement sercahghh11 =wait.until(ExpectedConditions.elementToBeClickable(sercahghh));
		sercahghh11.click();
		//Thread.sleep(3000);

		WebElement selectreco1h = wait.until(ExpectedConditions.elementToBeClickable(selectreco));
		selectreco1h.click();
		WebElement selectrecheckbojx1 = wait.until(ExpectedConditions.elementToBeClickable(selectrecheckbox));
		selectrecheckbojx1.click();
		WebElement Printandsample1h = wait.until(ExpectedConditions.elementToBeClickable(Printandsample));
		Printandsample1h.click();
		WebElement saveandsubmit1j = wait.until(ExpectedConditions.elementToBeClickable(saveandsubmit));
		saveandsubmit1j.click();
		//Thread.sleep(4000);
		String windowHandle64 = driver.getWindowHandle();
		ArrayList<Object> tabsr2u = new ArrayList<Object> (driver.getWindowHandles());
		driver.switchTo().window((String) tabsr2u.get(0));
		//Thread.sleep(2000);
		Thread.sleep(2000);
		WebElement SideBarrwf1 = wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarrwf1.click();
		//Thread.sleep(7000);
		Thread.sleep(2000);
		WebElement centerreceving1f = wait.until(ExpectedConditions.elementToBeClickable(centerreceving));
		centerreceving1f.click();
		//Thread.sleep(1000);
		String windowHandle643 = driver.getWindowHandle();
		ArrayList<Object> tabsr23u = new ArrayList<Object> (driver.getWindowHandles());
		driver.switchTo().window((String) tabsr23u.get(0));
		//Thread.sleep(1000);
		WebElement centerrecivecwhckf1 = wait.until(ExpectedConditions.elementToBeClickable(centerrecivecwhck));
		centerrecivecwhckf1.click();
		WebElement receiversubmift1 = wait.until(ExpectedConditions.elementToBeClickable(receiversubmit));
		receiversubmift1.click();
		WebElement receivesamplef1 = wait.until(ExpectedConditions.elementToBeClickable(receivesample));
		receivesamplef1.click();
		Thread.sleep(2000);
		WebElement SideBarr123= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr123.click();
		Thread.sleep(2000);
		WebElement Pathology14 = wait.until(ExpectedConditions.elementToBeClickable(Pathology));
		Pathology14.click();
		Thread.sleep(2000);
		WebElement fgg = wait.until(ExpectedConditions.elementToBeClickable(sampleprocessing));
		fgg.click();
		//Thread.sleep(3000);
		Thread.sleep(2000);
		WebElement checkclcikf1 = wait.until(ExpectedConditions.elementToBeClickable(checkclcik));
		checkclcikf1.click();
		WebElement Acceptsamplef1 = wait.until(ExpectedConditions.elementToBeClickable(Acceptsample));
		Acceptsamplef1.click();
		WebElement Acceptsamplesavfe1 = wait.until(ExpectedConditions.elementToBeClickable(Acceptsamplesave));
		Acceptsamplesavfe1.click();
		//Thread.sleep(3000);
		Thread.sleep(2000);
		WebElement SideBarrwf12 = wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarrwf12.click();
		//Thread.sleep(7000);
		Thread.sleep(2000);
		WebElement ResultEntry13= wait.until(ExpectedConditions.elementToBeClickable(ResultEntry));
		ResultEntry13.click();
		Thread.sleep(3000);
		String jj = driver.findElement(statuss).getText();
		String Actual = jj;
		String Expected = jj;
		Assert.assertEquals(Actual, Expected, "Rejected order is Accepted");



	}

	public void ResultEntryPrintWorklisy () throws InterruptedException {


		// Click on menu icon
		driver.findElement(mainmenu).click();
		Thread.sleep(3000);

		// Click on Lab in the menu
		driver.findElement(Lab).click();
		Thread.sleep(3000);

		// Move to the sidebar using Actions
		new Actions(driver).moveToElement(driver.findElement(SideBar)).build().perform();
		Thread.sleep(3000);

		// Click on the 'Microbiology' option
		driver.findElement(Microbilogy).click();
		Thread.sleep(3000);

		// Click on 'Result Entry Dashboard'
		driver.findElement(ResultEntryDashboard).click();
		Thread.sleep(3000);

		// Get MRN value from the element
		String mrnValue = driver.findElement(Getmrn).getText();
		Thread.sleep(3000);

		// Click on 'Expand' section
		driver.findElement(Expand).click();
		Thread.sleep(3000);

		// Send the MRN value to the search input field
		driver.findElement(mrnnn).sendKeys(mrnValue);
		Thread.sleep(3000);

		// Click on the search button
		driver.findElement(sercahghh).click();
		Thread.sleep(3000);

		String verifymrn = driver.findElement(getmrnn).getText();
		String Actualmrn = verifymrn;
		String Expectedmrn = verifymrn;
		Assert.assertEquals(Actualmrn, Expectedmrn, "MRN has been verified sucessfully");


	}



public void resultentryhold() throws InterruptedException {




		WebDriverWait wait = new WebDriverWait(driver,6000);
		/*
		/*

		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();


		Thread.sleep(8000); 

		driver.findElement(mainmenu2).click(); 
		Thread.sleep(8000);
		driver.findElement(OPD3).click(); 
		Thread.sleep(8000);

		//Thread.sleep(1000);
		driver.findElement(SideBarMenu).click();
		Thread.sleep(2000);
		driver.findElement(RegistrationMenu).click();
		Thread.sleep(2000); 
		driver.findElement(EncounterListw).click();
		Thread.sleep(3000);

		String takename = driver.findElement(getpataientname).getText();

		 */

		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();

		Thread.sleep(3000);
		WebElement Expand1 = wait.until(ExpectedConditions.elementToBeClickable(Expand)); 
		Expand1.click();
		//Thread.sleep(3000);
		WebElement mrnnn1 =wait.until(ExpectedConditions.elementToBeClickable(name));
		mrnnn1.sendKeys("PATHOLOGY"); 
		//Thread.sleep(3000);
		WebElement sercahghh1 =wait.until(ExpectedConditions.elementToBeClickable(sercahghh));
		sercahghh1.click();
		//Thread.sleep(3000);
		WebElement selectreco1 = wait.until(ExpectedConditions.elementToBeClickable(selectreco));
		selectreco1.click();
		WebElement selectrecheckbox1 = wait.until(ExpectedConditions.elementToBeClickable(selectrecheckbox));
		selectrecheckbox1.click();
		WebElement Printandsample1 = wait.until(ExpectedConditions.elementToBeClickable(Printandsample));
		Printandsample1.click();
		WebElement saveandsubmit1 = wait.until(ExpectedConditions.elementToBeClickable(saveandsubmit));
		saveandsubmit1.click();
		Thread.sleep(2000);
		String windowHandle6 = driver.getWindowHandle();
		ArrayList<Object> tabs2u = new ArrayList<Object> (driver.getWindowHandles());
		driver.switchTo().window((String) tabs2u.get(0));
		Thread.sleep(2000);

		WebElement SideBarr1 = wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr1.click();
		//Thread.sleep(7000);
		Thread.sleep(2000);
		WebElement centerreceving1 = wait.until(ExpectedConditions.elementToBeClickable(centerreceving));
		centerreceving1.click();
		driver.navigate().refresh();
		WebElement centerrecivecwhck1 = wait.until(ExpectedConditions.elementToBeClickable(centerrecivecwhck));
		centerrecivecwhck1.click();
		WebElement receiversubmit1 = wait.until(ExpectedConditions.elementToBeClickable(receiversubmit));
		receiversubmit1.click();
		WebElement receivesample1 = wait.until(ExpectedConditions.elementToBeClickable(receivesample));
		receivesample1.click();


		Thread.sleep(3000);

		WebElement SideBarr12= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr12.click();
		//Thread.sleep(7000);

		Thread.sleep(2000);

		WebElement Pathology1 = wait.until(ExpectedConditions.elementToBeClickable(Pathology));
		Pathology1.click();
		WebElement sampleprocessing1 = wait.until(ExpectedConditions.elementToBeClickable(sampleprocessing));
		sampleprocessing1.click();
		//Thread.sleep(3000);
		WebElement checkclcik1 = wait.until(ExpectedConditions.elementToBeClickable(checkclcik));
		checkclcik1.click();
		WebElement Acceptsample1 = wait.until(ExpectedConditions.elementToBeClickable(Acceptsample));
		Acceptsample1.click();
		WebElement Acceptsamplesave1 = wait.until(ExpectedConditions.elementToBeClickable(Acceptsamplesave));
		Acceptsamplesave1.click();

		Thread.sleep(2000);

		//	String kk = driver.findElement(getmrnre).getText();
		WebElement SideBarr13= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr13.click();
		Thread.sleep(2000);

		WebElement ResultEntry1= wait.until(ExpectedConditions.elementToBeClickable(ResultEntry));
		ResultEntry1.click();
		Thread.sleep(2000);

		WebElement checkboxdd1= wait.until(ExpectedConditions.elementToBeClickable(checkboxdd));
		checkboxdd1.click();
		Thread.sleep(2000);

		WebElement holdd1= wait.until(ExpectedConditions.elementToBeClickable(holdd));
		holdd1.click();
		Thread.sleep(2000);

		WebElement Confirmhold1= wait.until(ExpectedConditions.elementToBeClickable(Confirmhold));
		Confirmhold1.click();

		Thread.sleep(4000);

		String getdata = driver.findElement(getholddata).getText();

		String Actaul = getdata;
		String Expected = getdata;

		Assert.assertEquals(Actaul, Expected, "Order has been holded sucessfully");

		//Thread.sleep(3000);
		Thread.sleep(2000);
		WebElement CheckboxToUnhold1= wait.until(ExpectedConditions.elementToBeClickable(CheckboxToUnhold));
		CheckboxToUnhold1.click();
		Thread.sleep(2000);
		WebElement Unbhold1= wait.until(ExpectedConditions.elementToBeClickable(Unbhold));
		Unbhold1.click();
		Thread.sleep(2000);
		WebElement confirmunhold1= wait.until(ExpectedConditions.elementToBeClickable(confirmunhold));
		confirmunhold1.click();

		Thread.sleep(4000);

		String data = driver.findElement(getdataAccepted).getText();

		String Actual1  = data;
		String Expected1 = data;

		Assert.assertEquals(Actual1, Expected1 , "Unhold order is accepted");


	}


public void  Approval_Submit_Report() throws InterruptedException, IOException, AWTException 
	{	 



		WebDriverWait wait = new WebDriverWait(driver,6000);



		Thread.sleep(8000); 
		driver.findElement(mainmenu2).click(); 
		Thread.sleep(8000);
		driver.findElement(OPD3).click(); 
		Thread.sleep(8000);

		//Thread.sleep(1000);
		driver.findElement(SideBarMenu).click();
		Thread.sleep(2000);
		driver.findElement(RegistrationMenu).click();
		Thread.sleep(2000); 
		driver.findElement(EncounterListw).click();
		Thread.sleep(3000);

		String takename = driver.findElement(getpataientname).getText();



		driver.findElement(mainmenu).click();
		Thread.sleep(8000);
		driver.findElement(Lab).click();
		Thread.sleep(8000);


		Thread.sleep(3000);
		WebElement Expand1 = wait.until(ExpectedConditions.elementToBeClickable(Expand)); 
		Expand1.click();
		Thread.sleep(3000);
		WebElement mrnnn1 =wait.until(ExpectedConditions.elementToBeClickable(name));
		mrnnn1.sendKeys(takename); 
		Thread.sleep(3000);
		WebElement sercahghh1 =wait.until(ExpectedConditions.elementToBeClickable(sercahghh));
		sercahghh1.click();
		Thread.sleep(3000);
		WebElement selectreco1 = wait.until(ExpectedConditions.elementToBeClickable(selectreco));
		selectreco1.click();
		WebElement selectrecheckbox1 = wait.until(ExpectedConditions.elementToBeClickable(selectrecheckbox));
		selectrecheckbox1.click();
		WebElement Printandsample1 = wait.until(ExpectedConditions.elementToBeClickable(Printandsample));
		Printandsample1.click();
		WebElement saveandsubmit1 = wait.until(ExpectedConditions.elementToBeClickable(saveandsubmit));
		saveandsubmit1.click();
		Thread.sleep(2000);
		String windowHandle6 = driver.getWindowHandle();
		ArrayList<Object> tabs2u = new ArrayList<Object> (driver.getWindowHandles());
		driver.switchTo().window((String) tabs2u.get(0));
		Thread.sleep(2000);

		WebElement SideBarr1 = wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr1.click();
		Thread.sleep(7000);
		WebElement centerreceving1 = wait.until(ExpectedConditions.elementToBeClickable(centerreceving));
		centerreceving1.click();
		WebElement centerrecivecwhck1 = wait.until(ExpectedConditions.elementToBeClickable(centerrecivecwhck));
		centerrecivecwhck1.click();
		WebElement receiversubmit1 = wait.until(ExpectedConditions.elementToBeClickable(receiversubmit));
		receiversubmit1.click();
		WebElement receivesample1 = wait.until(ExpectedConditions.elementToBeClickable(receivesample));
		receivesample1.click();


		Thread.sleep(3000);

		WebElement SideBarr12= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr12.click();
		Thread.sleep(7000);

		WebElement Pathology1 = wait.until(ExpectedConditions.elementToBeClickable(Pathology));
		Pathology1.click();
		WebElement sampleprocessing1 = wait.until(ExpectedConditions.elementToBeClickable(sampleprocessing));
		sampleprocessing1.click();
		Thread.sleep(3000);
		WebElement checkclcik1 = wait.until(ExpectedConditions.elementToBeClickable(checkclcik));
		checkclcik1.click();
		WebElement Acceptsample1 = wait.until(ExpectedConditions.elementToBeClickable(Acceptsample));
		Acceptsample1.click();
		WebElement Acceptsamplesave1 = wait.until(ExpectedConditions.elementToBeClickable(Acceptsamplesave));
		Acceptsamplesave1.click();




		Thread.sleep(3000);

		//	String kk = driver.findElement(getmrnre).getText();
		WebElement SideBarr13= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr13.click();
		Thread.sleep(5000);


		WebElement Microbilogy11 = wait.until(ExpectedConditions.elementToBeClickable(Microbilogy));
		Microbilogy11.click();
		//driver.findElement(Microbilogy).click();
		WebElement MediaObservation1 = wait.until(ExpectedConditions.elementToBeClickable(MediaObservation));
		MediaObservation1.click();
		Thread.sleep(7000);



		//driver.findElement(MediaObservation).click();
		WebElement Expand12 = wait.until(ExpectedConditions.elementToBeClickable(clickonedit));
		Expand12.click();




		//WebElement sercahghh13 = wait.until(ExpectedConditions.elementToBeClickable(edit));
		//sercahghh13.click();		
		WebElement Growth1 = wait.until(ExpectedConditions.elementToBeClickable(Growth));
		Growth1.click();
		//driver.findElement(Growth).click();
		WebElement Growth11 = wait.until(ExpectedConditions.elementToBeClickable(Growth12));
		Growth11.click();
		//	driver.findElement(nogroth).click();
		WebElement status1 = wait.until(ExpectedConditions.elementToBeClickable(status));
		status1.click();
		//	driver.findElement(status).click();
		WebElement Pending1 = wait.until(ExpectedConditions.elementToBeClickable(Completed));
		Pending1.click();




		WebElement growthrwosec1 = wait.until(ExpectedConditions.elementToBeClickable(growthrwosec));
		growthrwosec1.click();
		//driver.findElement(Growth).click();
		WebElement Growth119 = wait.until(ExpectedConditions.elementToBeClickable(Growth12));
		Growth119.click();
		//	driver.findElement(nogroth).click();
		WebElement status17 = wait.until(ExpectedConditions.elementToBeClickable(stats2));
		status17.click();
		//	driver.findElement(status).click();
		WebElement Pending17 = wait.until(ExpectedConditions.elementToBeClickable(Completed));
		Pending17.click();



		WebElement cultur1 = wait.until(ExpectedConditions.elementToBeClickable(cultur));
		cultur1.click();
		Thread.sleep(5000);
		//nogroth1.click();
		WebElement nogroth13 = wait.until(ExpectedConditions.elementToBeClickable(Growth12));
		nogroth13.click();
		//driver.findElement(cultur).click();
		WebElement status21 = wait.until(ExpectedConditions.elementToBeClickable(status2));
		status21.click();
		WebElement Pending21 = wait.until(ExpectedConditions.elementToBeClickable(Completed));
		Pending21.click();
		//driver.findElement(status2).click();
		WebElement savesample1 = wait.until(ExpectedConditions.elementToBeClickable(savesample));
		savesample1.click();
		//driver.findElement(savesample).click();
		WebElement Yess1 = wait.until(ExpectedConditions.elementToBeClickable(Yess));
		Yess1.click();
		Thread.sleep(2000);
		WebElement SideBarr17= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarr17.click();
		Thread.sleep(2000);



		WebElement Microbilogy15 = wait.until(ExpectedConditions.elementToBeClickable(Microbilogy));
		Microbilogy15.click();
		Thread.sleep(2000);

		WebElement resultentry2 = wait.until(ExpectedConditions.elementToBeClickable(resultentry));
		resultentry2.click();


		Thread.sleep(2000);
		WebElement sercahghh138 = wait.until(ExpectedConditions.elementToBeClickable(resultentydetailseidt));
		sercahghh138.click();
		Thread.sleep(2000);
		WebElement Observation1 = wait.until(ExpectedConditions.elementToBeClickable(Observation));
		Observation1.click();
		WebElement pushcell1 = wait.until(ExpectedConditions.elementToBeClickable(pushcell));
		pushcell1.click();
		WebElement response1 = wait.until(ExpectedConditions.elementToBeClickable(response));
		response1.click();
		WebElement formula1 = wait.until(ExpectedConditions.elementToBeClickable(formula));
		formula1.click();
		WebElement addformula1 = wait.until(ExpectedConditions.elementToBeClickable(addformula));
		addformula1.click();
		WebElement Identificationtab1 = wait.until(ExpectedConditions.elementToBeClickable(Identificationtab));
		Identificationtab1.click();
		WebElement organism1 = wait.until(ExpectedConditions.elementToBeClickable(organism));
		organism1.click();
		WebElement input1 = wait.until(ExpectedConditions.elementToBeClickable(input));
		input1.sendKeys("Staphylococci");
		WebElement Sythaphil1 = wait.until(ExpectedConditions.elementToBeClickable(Sythaphil));
		Sythaphil1.click();
		Thread.sleep(3000);
		WebElement Panel18 = wait.until(ExpectedConditions.elementToBeClickable(Panel));
		Panel18.click();
		WebElement des_31 = wait.until(ExpectedConditions.elementToBeClickable(des_3));
		des_31.click();
		//	WebElement descopt1 = wait.until(ExpectedConditions.elementToBeClickable(descopt));
		//	descopt1.click();
		WebElement method1 = wait.until(ExpectedConditions.elementToBeClickable(method));
		method1.click();
		WebElement cultureandsensitive1 = wait.until(ExpectedConditions.elementToBeClickable(cultureandsensitive));
		cultureandsensitive1.click();
		WebElement cafezai1 = wait.until(ExpectedConditions.elementToBeClickable(cafezai));
		cafezai1.sendKeys("1");
		WebElement Interpetation1 = wait.until(ExpectedConditions.elementToBeClickable(Interpetation));
		Interpetation1.click();
		WebElement interpretopt1 = wait.until(ExpectedConditions.elementToBeClickable(interpretopt));
		interpretopt1.click();
		WebElement savesampleidenti1 = wait.until(ExpectedConditions.elementToBeClickable(savesampleidenti));
		savesampleidenti1.click();
		Thread.sleep(2000);
		//	String kk = driver.findElement(getmrnre).getText();
		WebElement SideBarur13= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarur13.click();

		Thread.sleep(8000);
		//WebElement Microbilogy11 = wait.until(ExpectedConditions.elementToBeClickable(Microbilogy));
		//Microbilogy11.click();

		//   Thread.sleep(5000);

		WebElement ResultEntryVerification1= wait.until(ExpectedConditions.elementToBeClickable(ResultEntryVerification));
		ResultEntryVerification1.click();
		WebElement EditResultverification2= wait.until(ExpectedConditions.elementToBeClickable(EditResultverification));
		EditResultverification2.click();
		Thread.sleep(2000);
		WebElement SaveSubbmit1= wait.until(ExpectedConditions.elementToBeClickable(SaveSubbmit));
		SaveSubbmit1.click();
		Thread.sleep(2000);
		WebElement SideBarur7138= wait.until(ExpectedConditions.elementToBeClickable(SideBarr));
		SideBarur7138.click();

		WebElement Microbilogy191 = wait.until(ExpectedConditions.elementToBeClickable(Microbilogy));
		Microbilogy191.click();

		Thread.sleep(2000);
		WebElement ResultApproval3= wait.until(ExpectedConditions.elementToBeClickable(ResultApproval));
		ResultApproval3.click();
		Thread.sleep(2000);
		WebElement Editresultapprov23= wait.until(ExpectedConditions.elementToBeClickable(Editresultapprov));
		Editresultapprov23.click();
		Thread.sleep(2000);
		WebElement Savesugbmit1= wait.until(ExpectedConditions.elementToBeClickable(Savesugbmit));
		Savesugbmit1.click();

	}

	



}
