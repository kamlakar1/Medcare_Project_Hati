...
public class Billing_Company_Deposit_DebitCard_Save 
{
	WebDriver driver;
	...
	By CardTypeSelOpt=By.xpath("//span[contains(text(),'VISA')]");
	//Bank
	By BankClick=By.cssSelector("mat-select[ng-reflect-name='bank']");
	By BankSelOpt=By.xpath("//span[contains(text(),'CIMB Group Holdings')]");
	//Approval Code
	By ApprovalCOdeClick=By.cssSelector("input[ng-reflect-name='approvalCode']");
	public void Billing_COmpany_Deposit_for_DebitCard() throws InterruptedException
	{
		...
		Thread.sleep(2000);
		driver.findElement(PaymentTypeSelOpt).click();
	...
	public Billing_Company_Deposit_DebitCard_Save(WebDriver driver)
	{
		this.driver=driver;
	}
}
